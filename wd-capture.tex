...
We focus here on the thermalization and self-gravitational collapse of DM, assuming that the DM loses a negligible amount of energy during a single transit:
\begin{equation}
...
In this case we should formally take $N_\text{sg} = 2$, and our DM ``core" is more properly described as a $2$-body gravitational bound state within the star. 
We do not explicitly consider such $2$-body ``cores" in this paper, although they are subject to the same decay constraints as $N$-body cores and will collapse via orbital decay (in the absence of a stabilizing DM-DM force) on the timescale $t_\x{cool}$, yielding an analogous annihilation constraint. 
