...
We review here the conditions necessary for an energy deposition event in a WD to result in a runaway fusion and a type Ia SN.

A general local energy deposit to a WD will, after an appropriate thermalization time, heat a region within the star. 
Since we are interested in inciting nuclear fusion, we focus on the heating of the ions and take this heated region to correspond to the local peak in ion temperature that forms as a result of the deposited energy. 
We will parameterize any such event in terms of the localized ion heating that it produces - let $L_0$ be the length scale of the heated region and $\Ez$ be the excess thermal energy that it contains.
This corresponds to an ion temperature of $T_0 \sim \Ez L_0^3 n_\text{ion}$ within the heated region, where $n_\text{ion}$ is the number density of ions.
Note that the size and thermal properties of the heated region will evolve in time, but we take the above parameters to refer to it as it initially forms, i.e.\ immediately after the deposited energy has thermailzed with the ions.  

The heating length $L_0$ for a given deposition event is thus a measure of the efficiency with which the deposited energy is transfered to the ions.
...
The fate of a localized heated region is either a non-violent diffusion of the deposited energy across the star, or a runaway fusion chain-reaction that destroys the star.
The outcome of a given heating event is governed by two WD parameters, a \emph{fusion temperature} $T_f$ and a \emph{trigger size} $\lambda_T$.
...
In carbon-oxygen WDs, to which we restrict this analysis, the fusion temperature is a constant $T_f \sim \MeV$. \textcolor{blue}{citation?}

While any region with $T_0 > T_f$ will initially support fusion, this condition is not sufficient for triggering a runaway. 
Sustained heating via fusion can only be maintained if the timescale for this heating is much shorter than the region's cooling timescale.
...
Depending on the details of the initial distribution, this interaction may give rise to one temperature peak involving the energy of many produced particles, or a number of smaller, separated peaks.
 In any case, the energy of these deposits $\Ez$ will be of order the initial kinetic energy of the SM particles involved.
...
Once $\Ez$ and $L_0$ have been computed in this manner, the explosion condition is determined by \eqref{eq:boom}.
