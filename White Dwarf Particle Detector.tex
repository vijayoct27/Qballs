...
This information is needed to determine if a given DM encounter with a WD results in runaway fusion. 
...
We make some simplifying assumptions about the spatial extent of these interactions.
...
In the DM capture scenario, the energy loss due to DM-SM scatters is sufficient to stop the DM and cause it to accumulate inside the star, but not enough to trigger runaway. 
We consider both scenarios for collision and decay candidates, for which the capture scenario has a significantly enhanced rate of explosive events. 
