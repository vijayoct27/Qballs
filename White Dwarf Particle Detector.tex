...
We consider a general class of models in which DM-DM collisions, DM decays, or DM transits with a SM scattering interaction produce particles that subsequently deposit energy inside the star.  
...
Many terrestrial direct detection experiments are designed to search for DM \cite{Akerib:2016vxi, Agnese:2017njq}, yet these lose sensitivity to heavier DM due to its diminished number density and local flux on earth.
...
Currently there is a wide range of unexplored parameter space for DM candidates less than $\sim 10^{48} ~\GeV$, at which point the DM will have observable gravitational microlensing effects \cite{Griest:2013aaa}. 
...
Thermonuclear runaway fusion requires both a heating event and the lack of significant cooling which might quench the process.
The WD medium is particularly suited for this as it is dominated by degeneracy pressure and undergoes minimal thermal expansion, which is the mechanism that regulates fusion in main sequence stars.
Thermal diffusion is the primary cooling process in a WD and can be thwarted by heating a large enough region.
The properties of a localized heating necessary to trigger runaway fusion was computed in \cite{Woosley}.
Consequently, \cite{Graham:2015apa} realized that if DM were capable of sufficiently heating a WD in this manner, it would result in a SN with sub-Chandrasekhar Mass progenitor. 
...
In this paper, we examine DM candidates with non-gravitational interactions that cause heating through the release of SM particles. 
An essential ingredient in this analysis is calculating the length scales over which different particle species deposit their energy in a WD medium. 
We find that most high energy particles tend to thermalize efficiently with ions in the WD, nearly independent of species or initial energy.
...
We consider a general class of models in which DM-DM collisions, DM decays, or DM transits with a SM scattering interaction produce particles that subsequently deposit energy inside the star. 
Constraints on these models come from either observing specific, long-lived WDs or by comparing the measured type 1a SN rate with the rate expected due to DM. 
...
We are thus able to use WDs to rule out supersymmetric Q-ball DM in a vast region of its parameter space. 
It is important to note that these constraints are complimentary to terrestrial ones---it is more massive DM that is likely to trigger SN, but also more massive DM that has low terrestrial flux.
...
In Section~\ref{sec:SMHeating} we study the non-gravitational heating of a WD due to the release of high-energy SM particles. 
Detailed calculations of the stopping of such particles are provided in Appendix \ref{sec:Appendix}.
In Section~\ref{sec:DMexplode} we parameterize the explosiveness and rate of events for the large class of DM-WD encounters that we consider. 
In Section~\ref{sec:Constraints} we derive schematic constraints on such models using WD observables. 
We specify to the case of Q-ball DM in Section~\ref{sec:QBalls}, and conclude in Section~\ref{sec:Discussion}.

\section{White Dwarf Runaway Fusion}
\label{sec:Review}

We first review the conditions for which a local energy deposit in a WD results in runaway fusion.
...
Thus, for a given temperature greater than $T_f$, there is always a critical size above which the heated region does not cool and initiates nuclear runaway.
...
As in \cite{Graham:2015apa}, we restrict our attention to carbon-oxygen WDs in the upper mass range $\sim 0.85 - 1.4 ~M_{\odot}$---we will eventually see that only these give meaningful constraints on DM.
...
However, this may still result in thermonuclear runaway if the temperature remains of order $T_f$ or greater by the time the region has diffused out to the trigger size.
...
The relation between $\Ez$ and $T_0$ depends on which WD constituents (ions, degenerate electrons, and photons) are heated.
...
\end{align}
where we have assumed the WD has roughly constant density within $R_\text{vir}$.
...
\label{eq:tcol}
  t_\text{collect}
  &\sim \frac{\rho_\text{WD} R_\text{vir}^3}
...
\begin{align}
  t_\text{collapse} &\sim R_\text{vir}/v_\text{th} \\
