...
Any energy deposit will eventually heat ions within some localized region---parameterize this region by a linear size $L_0$ and peak temperature $T_0$.
Of course these scales evolve in time, but it will be useful to describe a given heating event by their initial values.
...
Any event with $T_0 > T_f$ will initially support fusion, although this is not sufficient for triggering runaway as cooling processes may rapidly lower the temperature below $T_f$.
This cooling will not occur if the corresponding cooling timescale is larger than the timescale at which fusion releases energy.
...
Thus for a given temperature greater than $T_f$, there is a critical size above which the heated region does not cool and initiates nuclear runaway.
...
As in \cite{Graham:2015apa} we restrict our attention to carbon-oxygen WDs in the upper mass range $\sim 0.85 - 1.4 ~M_{\odot}$---these will yield the most stringent constraints on DM.
...
However, this will still result in runaway fusion if the temperature is of order $T_f$ by the time the region has diffused out to the trigger size.
...
\end{align}
<<<<<<< HEAD
It will be useful for our purposes to phrase this condition in terms of the total energy $\Ez$ deposited during a heating event.
The relation between $\Ez$ and $T_0$ depends on the efficiency with which WD constituents (ions, electrons, and photons) interact and thermalize with each other. 
In particular, it is important to know the thermalization length scale over which each species transfers $\OO(1)$ of its energy.
As this only matters at a size $\lambda_T$ and temperature $T_f$, the question becomes: 
This is non-trivial, and we explicitly classify and calculate these interactions in Appendix \ref{sec:Appendix}. 
It is found that electrons 
The excess energy in a volume $V$ required to heat all these species to the critical temperature $T_f$ is given by a sum of their heat capacities
=======
...
The relation between $\Ez$ and $T_0$ depends on which WD constituents (ions, electrons, or photons) are heated.
Of course, these different species interact with each other.
We calculate this interaction and calculate the distance over which $\OO(1)$ of

to varying degrees each will may thermalize
...
Note that this contribution uses the heat capacity of a degenerate gas as the Fermi energy $E_F \gtrapprox T_f$ for the densities we consider.
 

In order for \eqref{eq:heatcapacity} to hold, it must be the case that 



We calculate this interaction and calculate the distance over which $\OO(1)$ of to varying degrees each will may thermalize 
The lengths over which they mutually thermalize when heated to $\sim T_f$ are plotted in \textcolor{blue}{figure}, and as these are all smaller than the trigger size we will have all three species in equilibrium during any event capable of runaway. 


The minimum energy deposit needed for ignition is
\begin{align}
\label{eq:Eboom}
...
\caption{Minimum energy $\Eboom$ \eqref{eq:Eboom} required to trigger supernovae as a function of WD mass, based on numerical results for $\lambda_T$ \cite{Woosley} and the WD mass-density relation \cite{cococubed}}.
\label{fig:Eboom}
\end{figure}
...
This is done in \cite{Graham:2015apa} for the case of primordial black holes which transit a WD and cause heating through dynamical friction.
Here we instead consider non-gravitational heating of a WD due to the DM.
In particular, any DM candidate which couples to the SM will generically be able to release SM particles inside the star which then thermalize with stellar constituents.
...
While this can be done precisely for a specific DM model, in this Section we consider more generally several simple, illustrative classes of DM-SM interactions which demonstrate the typical event rates and resulting constraints expected for ultra-heavy DM.
...
The stopping power for Coulomb collisions with ions is most easily understood if we describe these scatters in terms of the energy transfer $\omega$ or the momentum transfer $q$.
...
The scattering cross section is (e.g., \cite{Jackson})
\begin{align}
  \label{eq:StationaryCoulombCrossSection}
\frac{d \sigma}{d \omega} = \frac{2 \pi Z^2 \alpha^2}{m_\text{ion}\beta^2} \frac{1}{ \omega^2}
\end{align}
This cross section receives a correction for incident to spin-1/2 particles, but we will neglect this because it does not change our results appreciably.

Integrating this to obtain the stopping power, we find
\begin{align}
\frac{dE}{d x} &= \int_{\omega_\text{min}}^{\omega_\text{max}} d \omega \, n_\text{ion} \frac{d \sigma}{d \omega} \omega \nonumber\\
 &= \frac{2 \pi\, n_\text{ion} Z^2 \alpha^2 }{m_\text{ion}\beta^2} \log\left( \frac{\omega_\text{max}}{\omega_\text{min}} \right)
\end{align}
what we have specified the maximum and minimum allowed energy transfers $\omega_\text{max}$ and $\omega_\text{min}$.
...
\begin{align}
  \omega_\text{max} = \omega_\text{kin} = \frac{2 m_\text{ion} p^2}{m_\text{ion}^2 + m^2 + 2E m_\text{ion}}
\end{align}
where $p$, $E$ are the incoming momentum and energy.

The minimum energy transfer is set by plasma screening in the WD, which cuts off soft scatters by changing the effective Coulomb potential around an ion to be
\begin{align}
V_\text{eff}(\textbf{r}) = \frac{Z \alpha}{r} e^{-\lambda_\text{TF} r}
\end{align}
...
To see how $\omega_\text{min}$ emerges from this, consider for a moment the Born approximation at low incident energies where the ion remains non-relativistic and the scatters are effectively elastic.
It is at these low energies where small energy transfers near $\omega_\text{min}$ may comprise an appreciable fraction of the stopping power.
In terms of the momentum transfer $q$, the cross section from the Born approximation is
\begin{align}
\frac{d \sigma}{dq^2} = \frac{1}{4 \pi \beta^2} \left|\int d^3 r \, e^{i\textbf{q}\cdot \textbf{r}} V(\textbf{r}) \right|^2
\end{align}
In this regime the energy transfer is $\omega = q^2 / 2 m_\text{ion}$, and plugging in $V(\textbf{r}) = Z \alpha / r$ we recover \eqref{eq:StationaryCoulombCrossSection}. Inserting the screened potential, we find
\begin{align}
\frac{d \sigma}{dq^2} = \frac{4 \pi Z^2 \alpha^2}{\beta^2}\frac{1}{\left(q^2 + \lambda_\text{TF}^{-2}\right)^2}
\end{align}
