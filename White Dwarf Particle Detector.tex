...
The range of ions is given in \textcolor{blue}{figure} and that of electron bremsstrahlung and Compton scattering in \textcolor{blue}{figure}. Note these are all below the trigger size at energies $~\MeV$.
...
Since the heat capacity at these temperatures is dominated by photons, with energy density $ \sim T^4$, diffusion of a fixed energy deposit from an initial volume of length scale $\lambda_1$ to a larger final volume of length scale $\lambda_2$ decreases the temperature as $T_2 \sim T_1(\lambda_1 / \lambda_2)^{3/4}$.
Inspecting Figures~\ref{fig:SPhighEM} and~\ref{fig:SPlowEM} at the EM shower cut off $E_\text{crit} \sim 100~\text{MeV}$, at high and low densities we find $\lambda_\text{brem} / \lambda_{\gamma-\text{nuc}} \sim 10^{-1}$ and~$10^{-2}$, respectively.
Thus if a shower begins at high energies and terminates at $T_1 \sim E_\text{crit}$ and $\lambda_1 \sim \lambda_\text{brem}$, the final temperature once it has defused to $\lambda_2 \sim \lambda_{\gamma-\text{nuc}}$ is $T_2 \sim 20~\text{MeV}$ at high densities and $T_2 \sim 3~\text{MeV}$ at low densities.
Showers beginning at lower energies will diffuse in a similar way, but with different starting temperatures and sizes.
...
This range is plotted in \textcolor{blue}{figure}, where it is seen that for EM gas temperatures $1-10~\MeV$ the electrons thermalize over a distance of order the trigger size.
