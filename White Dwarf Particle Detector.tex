...
Identifying DM remains one of the clearest paths beyond the Standard Model, so it is fruitful to study the observable signatures of any yet-allowed candidate.
Many experiments are designed to detect DM at the weak scale or lighter, yet these or any terrestrial search is unable to probe ultra-heavy candidates due to the small local flux. 
Even for a strongly interacting candidate, if the mass is above $10^{22} ~\GeV$ a large detector of $(100 ~\text{m})^2$ will register fewer than a one event per year.
While this mass is outrageous on particle scales, there is no model-independent constraint on masses below roughly $10^{19} ~\text{g} \approx 10^{40} ~\GeV$ \textcolor{blue}{citation} and it is reasonable to suppose that such ultra-heavy DM may exist as composite states, just as the SM produces complex structures with mass much larger than fundamental scales (e.g., you, dear reader). 
This work will present signatures generic to DM with masses above $10^{16} ~\GeV$ and sufficient non-gravitational interactions, as well as some specific bounds on such models. 

For ultra-heavy DM, the natural arena of detection is astrophysics. 
One signal proposed by \cite{Graham:2015apa} is the triggering of supernovae (SN) in sub-Chandrasekhar white dwarf (WD) stars by DM-induced runaway fusion.
Igniting runaway fusion requires both a heating event and the lack of significant cooling mechanisms which might quench the process. 


The WD medium is particularly suited to this.
Being dominated by degeneracy pressure, it undergoes minimal thermal expansion, and due to the large number density thermal diffusion due to the high density. 
is the dominant WD cooling mechanism, which can be thwarted by igniting fusion within a large enough region. 
This critical size is actually quite small, due to the very high density and there inefficient diffusion within the WD medium. 



White dwarfs are particularly suited to this as their size is set by non-thermal degeneracy pressure and they are therefore unable to cool via thermal expansion as do main sequence stars. 
Thermal diffusion is the dominant WD cooling mechanism, which can be thwarted by igniting fusion within a large enough region. 
This critical size is actually quite small, due to the very high density and there inefficient diffusion within the WD medium. 
...
% Figures \ref{fig:SPelectron}, \ref{fig:SPphoton}, \ref{fig:SPnuc}, and \ref{fig:SPpion}.
...
For $M > 0.85 M_{\odot}$ the electrons are relativistic and have Fermi energy
\begin{equation}
  E_F \sim (3 \pi^2 n_e)^{1/3} \sim \OO\left(10 ~\MeV\right).
\end{equation}
\textcolor{blue}{check Efermi value}
This is significantly larger then the WD thermal temperature $T \sim \keV$ \textcolor{blue}{CITATION}, so we will take the electrons to have $T=0$. 

The carbon nuclei are fully-ionized, non-degenerate gas at the thermal temperature. 
Compared to the incident particles of interest, these ions are nearly at rest.
...
\label{eq:lattice}
  \Omega_l \sim \frac{Z^2 \alpha}{n_\text{ion}^{-1/3}}
         ...
         \gg T,
\end{equation}
\textcolor{blue}{check $\Omega_l$ - it does not agree with PR's later quoted lattice energies}
so they form a Coulomb lattice with binding energy $\Omega_l$.
The star also contains thermal photons, though they never play a dominant role in stopping as their number density $n_\text{photon} \sim T^3$ is much smaller than that of ions and electrons.
...
\subsection{Ionic Coulomb Collisions}
...
Coulomb scattering off Fermi electrons has two additional features compared to scattering off ions: the electron targets are not stationary and they require a threshold energy transfer in order to be scattered out of the Fermi sea.
