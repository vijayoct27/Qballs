...
For incident hadrons of energy $\epsilon > 10 ~\text{MeV}$, the energy loss is dominated by inelastic nuclear collisions in which the incoming hadron strongly interacts with a target nucleus to produce an $\OO(1)$ number of secondary hadrons.
...
Note that neutral pions decay to photons with a mean lifetime $\sim 10^{-16} ~\text{s}$. 
A $10 ~\text{MeV}$ neutral pion has a decay length of order $\sim 10^{-6} ~\text{cm}$ while more relativistic $\pi^0$ can travel much further.
...
For simplicity, we will ignore all electromagnetic shower contributions and focus simply on the hadronic component, which will carry a large fraction of the deposited energy.  

Although a hadronic shower initially contains mostly pions, the final-state hadrons will consist of roughly equal fractions of pions, protons, and neutrons. 
At these energies, protons and neutrons are stopped predominantly by elastic nuclear scatters, characterized by a mean free path $l_\text{el}$ and cross section of order $\sigma_\text{el} \approx 1 ~\text{b}$. 
...
These electrons can acquire a temperature at most just above the Fermi energy and therefore disperse their energy to ions over a much larger distance than any of the hadronic interaction lengths considered. 
However, since each nucleon and pion species carries an $\OO(1)$ fraction of the initial energy, we may simply ignore the energy lost into electrons.
As a result, thermalization of ions will occur after $\sim 10 ~\text{MeV}$ protons and neutrons have transferred most of their energy through elastic nuclear scattering. 
We find that $\sim 10$ random-walk collisions are needed to slow such nucleons to $\sim \MeV$ energies, resulting in a final deposition length of order
\begin{equation}
 \lambda_\text{el} \sim \sqrt{10} \times l_\text{el} \approx 10^{-7} ~\text{cm} \l\frac{10^{32}~\text{cm}^{-3}}{n_\text{ion}}\r
\end{equation}
If only a single high-energy hadron $N \sim 1$ is released in the WD, the resulting ion temperature profile will be displaced from the origin by a distance $X_\text{had}$ with characteristic size $L_0 \sim \lambda_\text{el}$. 
