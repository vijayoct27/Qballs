...
The stopping power is given by integrating over all targets transverse to the incident particle direction
\begin{align}
...
The screening length scale $\lambda_{TF}$ is given in the Thomas-Fermi approximation by \cite{Teukolsky}
\begin{align}
\label{eq:TF}
...
\begin{align}
  \frac{dE}{dx} &\approx n_\text{ion} 
    \int_0^{\lambda_{TF}} \, 2\pi b  \, db \cdot \omega \\ 
    &\approx 2\pi n_\text{ion}\left(\omega_\text{kin}\int_0^{b_0} \, db \, b 
    + \frac{\alpha^2 Z^2}{2 m_c \beta^2}  
    \int_{b_0}^{\lambda_{TF}} \,\frac{db}{b} \right)  \nonumber \\
    &= \begin{dcases}
      n_\text{ion} \frac{\pi \alpha^2 Z^2}{2 m_c \beta^2} 
      \frac{\lambda_{TF}^2}{b_0^2} 
        & \lambda_{TF} < b_0 \\
      n_\text{ion} \frac{\pi \alpha^2 Z^2}{2 m_c \beta^2} \left[ 1 
        + \log\left(\frac{\lambda_{TF}^2}{b_0^2}\right)
        \right] & \lambda_{TF} > b_0. \nonumber 
  \end{dcases}
\end{align}
...
This qualitatively changes the stopping power at low energies.

Consider first the limit of a non-relativistic incident particle of mass $m \gg m_e$, charge number $Z$, and incident momentum $\vec{p}$. 
As the electron speeds are much faster than this incident, a target electron with momentum $\vec{q}$ will scatter off the incident to leading order with only a change in direction, 
\begin{align}
...
\end{align}
At leading order the electron is not aware of the small ion velocity, so this scattering occurs with the recoilless Mott cross section
\begin{align}
    \frac{d\sigma}{d\cos\theta} \approx \frac{\alpha^2 Z^2}{4 q^2}
    \frac{1}{\sin^4\left(\frac{\theta}{2}\right)} 
    \left[1 - \sin^2\left(\frac{\theta}{2}\right) \right]    
\end{align}
where we have taken the electron speed to be nearly $1$ and $\cos\theta = \hat{q}_{out} \cdot \hat{q}_{in}$. 
The incident particle will lose energy off electrons $\vec{q}$ at a rate
\begin{align}
  \frac{dE}{dt} &\approx dn \int d\left(\cos\theta\right) 
    \frac{d\sigma}{d\cos\theta} \omega \, 
    \Theta\left(\omega - E_f + q\right)
\end{align}
...
Note that the stopping power is given by $v_\text{ion}^{-1} (dE/dt)$.
Now summing over all target electrons with the Fermi distribution 
\begin{align}
  \frac{dn}{d^3q} = n_e \frac{3}{4\pi q_f^3} \, \Theta(q_f - q)
\end{align}
gives the full stopping power
\begin{align}
  \frac{dE}{dx} \approx n_e \frac{3 \alpha^2 Z^2}{8\pi q_f^3} 
  \int &d\left(\cos\theta_{out}\right) \, d^3q_{in} \, \frac{1}{q} \Bigg[ \\
   \, &\frac{1 - \sin^2\left(\frac{\theta}{2}\right)}
          {\sin^4\left(\frac{\theta}{2}\right)} 
   \, \hat{p} \cdot \left(\hat{q}_{out} - \hat{q}_{in}\right) \\
   \, &\Theta\left(\omega - E_f + q\right) \Theta(q_f - q) \Bigg]
\end{align}

The electrons are predominantly relativistic, and this looks li and moving much faster than the incident, this proceeds
This scattering is qualitativley similar 

The effect of Pauli blocking is similar to that of plasma screening discussed above - both prevent the softest scatters from occurring. 
The Pauli effect will suppress scattering with energy transfers roughly less than the Fermi energy. 
Plasma screening will suppress scatters below
\begin{align}
  \label{eq:omega_soft}
  \omega \approx \frac{\alpha^2 Z^2}{2 m_c \beta^2 \lambda_{TF}^2} \\
    \approx \frac{3 \pi \alpha^3 Z^2}{m_e \beta^2}\frac{\alpha n_e}{E_F}
\end{align} 
