...
The showering processes described above conclude with a cloud of $\sim 10~\MeV$ neutrons, protons, and charged pions or $\sim 10-10^4~\MeV$ electrons and photons. 
At these energies, Coulomb, Compton, and elastic nuclear scatters are the dominant processes. 
The ranges $\lambda$ set by these interactions are shown in \textcolor{blue}{figures}.

\paragraph{Neutrons.}
Neutronsare the simplest species, interacting only via elastic nuclear scatters.   
These are stronger than the inelastic interactions, $\sigma_\text{el} \approx 1 ~\text{b}$ \textcolor{blue}{cite something}. 
In addition, the mass hierarchy between carbon ions and neutrons requires $\sim 10$ scatters to transfer the hadron's energy, further suppressing the range by a random-walk factor,
\begin{align}
...
\paragraph{Thermalization of an EM Bath}
