...
We are thus sensitive to ultra-heavy DM states which interact in the WD to produce either a few GUT-scale particles or a very large number of weak-scale particles. 
...
We quantify the heating due to $N$ particles of single species $i$ and uniform energy $\epsilon$ released isotropically in the WD interior by $\Delta_i(N, \epsilon)$, the width of the resulting ion temperature peak that initially forms. 
...
An essential ingredient in the calculation of $\Delta_i(N,\epsilon)$ is the stopping power $(dE/dx)$ of individual SM species due to interactions with stellar constituents.  
...
Consider the release of low-energy light hadrons, $\epsilon \lesssim 10~\MeV$.
...
For charged pions, however, the energy is predominantly transfered into the degenerate WD electrons via Coulomb scatters, and so the question now depends on the length scale for these electrons to transfer their energy to the ions. 
As shown in Figure \ref{fig:SOMETHING}, below $10~\MeV$ there are effectively no processes that efficiently transfer energy from the electrons to the ions and so the ion heating will occur over a negligibly large distance. 

The behavior of these low-energy hadrons is important both as a possible SM secondary in a DM-WD encounter, but also as the end-state of the cooling of very energetic hadrons. 
Turning to hadrons with $\epsilon \gtrsim 10~\text{MeV}$, the energy loss of these particles is dominated by nonelastic nuclear collisions.
...
However, consider a simplified interaction in which $\sigma_{i,\epsilon}$ denotes the cross section for DM to scatter off a stellar constituent, producing $N$ SM particles of species $i$ and each with energy $\epsilon$.
...
Elastic electromagnetic interactions can be a dominant source of energy loss for charged particles scattering off ions or degenerate electrons in the WD. 
...
\begin{equation}
\label{eq:QED}
\frac{d \sigma}{dT} = \frac{2 \pi  \alpha^2 Z^2}{m_e \beta^2} \frac{1}{W^2} \l1-\beta^2 \frac{W}{E_\text{kin}} \r,
\end{equation}
...
\end{align}
We take the upper limit $E_\text{max}$ to be the maximum energy transfer allowed kinematically \eqref{eq:ekin}. 
...
n_e(W) = \left\{
        \begin{array}{ll}
            \displaystyle \int \limits_{E_F -T}^{E_F}dE ~g(E) & \quad E_F > T \\
            n_e & \quad E_F < T
        \end{array}
    \right.,
\end{equation}
where $g(E)$ is the density of states per unit volume for a three-dimensional free electron gas.
The effect of degeneracy can also be cast as a suppression of the differential cross section of order $\mathcal{O}(T/E_F)$ whenever energy less than the Fermi energy is transferred.
...
\end{equation}
with an appropriate Lorentz boost to the electron rest frame.
This does not alter the result beyond $\OO(1)$ factors since the degenerate electrons are only slightly relativistic.
Further, Pauli-blocking of the target electrons is calculated using a modified number density as in \eqref{eq:pauliblocking}, although we find that degeneracy only introduces a significant suppression when $k \lesssim 10 ~\text{MeV}$.
This is to be expected since the interaction is dominated by hard, near-forward scatters.
...
As expected, we find that the inverse Compton stopping power is negligible as compared to other sources of energy loss such as Coulomb scattering. 
