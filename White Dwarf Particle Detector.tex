...
If dark matter (DM) is capable of sufficiently heating a local region of a white dwarf, it will trigger runaway fusion and ignite a type 1a supernova.
...
It is also intriguing that the DM-induced SN discussed in this work would provide an alternative mechanism of triggering supernovae from sub-Chandrasekhar Mass progenitors.
...
\label{eq:tcol}
  ...
  & \approx 10 ~\text{s} \l \frac{10^{16} ~\GeV}{m_\text{DM}} \r^{3/2} \l \frac{0.4 ~\GeV/\text{cm}^3}{\rho_\text{DM}} \r, ~~~~ n_\text{ion} \sim 10^{31} ~\cm^{-3}, \nonumber
\end{align}
...
The constraint on DM is derived by requiring that the expected time for an event to strike earth is less than the lifetime of the detector $\sim 10 ~\text{yr}$.
Curiously, we find that in the ``wind scenario", the resulting bounds from cosmic rays are within a couple orders of magnitude as those due to the observation of a local WD.
This coincidence can be seen explicitly by comparing the effective ``space-time volumes" for the systems. 
 A cosmic ray detector sees events within a space-time volume $\sim (R_\text{det}^2 R_\text{halo} \times 10 ~\text{yr})$ which is of order the WD space-time volume for decay events $\sim (R_\text{WD}^3 \times 10^9 ~\text{yr}$), not including the gravitational enhancement. 
