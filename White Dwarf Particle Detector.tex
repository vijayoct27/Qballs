...
Many terrestrial experiments are designed to detect DM at the weak scale or lighter \cite{Akerib:2016vxi, Agnese:2017njq}, yet these lose sensitivity to heavier DM due to its diminished number density and local flux. 
...
This unknown DM physics determines the distribution in momentum, species, and space of the SM particles produced in the star, which is required to determine if a DM-WD encounter results in a SN and further to calculate the rate of those SN. 
This may be done precisely for a particular DM model, as we do in Section~\ref{sec:QBalls} for Q-ball DM. 
In this section, however, will remain agnostic about the identity of DM and instead consider several illustrative modes of DM-SM interaction to demonstrate simply the constraints which can be obtained by considering DM-induced SN.
We describe these interactions below and then compute their DM-induced SN rates.

A DM candidate with SM interactions may be generically expected to interact with the WD medium through the three processed depicted in Figure \ref{fig:feynman}: DM-DM collisions, DM decays, and DM-SM scattering. 
Note that for ultra-heavy DM these processes may not be elementary interactions, but could rather be more complicated events analogous to the interactions of heavy nuclei. 

We classify DM candidates first by the spatial extent of the SM products resulting from the encounter. 
We consider either ``point" or ``continuous" heating of the WD medium. 
In ``point" encounters, all SM products are produced in a localized region (smaller than the trigger size) after a rare interaction, whereas `continuous" encounters produce particles uniformly along the DM trajectory due to many interactions.
DM-DM collisions and DM decays are both examples of ``point" heating, provided they directly produce SM products and not some population of long-lived dark states.  
DM-SM collisions can fall in either category, with a hard catastrophic scatter giving ``point" heating and repeated soft scatters ``continuous" heating. 

In addition, we further classify candidates according to the evolution of the DM itself inside the star.
...
We consider two simple limiting scenarios: ``DM wind" and ``DM capture".
...
In the case of a ``continuous" energy deposit this distinction is unimportant as ignition will either occur on the initial segment of the DM trajectory or not at all. 
However, ignition by ``point" depositions can be sensitive to the time spent in the WD (decays), presence of other DM particles (collision), etc., and thus models that undergo capture can be more strongly constrained.   
...
\subsection{Point Heating}

For a ``point" event releasing particles of heating length $L_0$, ignition will occur if the total energy in SM products satisfies condition~\eqref{eq:energy_boom_condition}. 
...
\end{equation}
We are thus sensitive to ``point" heating by DM with masses $m_\text{DM} \gtrsim 10^{16} ~\GeV$.


\subsection{Continuous Heating}
