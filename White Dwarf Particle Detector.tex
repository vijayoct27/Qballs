...
We quantify the heating due to $N$ particles of a single SM species and uniform energy $\epsilon$ released isotropically in the WD interior by $\Delta(N, \epsilon)$, the width of the resulting initial ion temperature peak. 
...
An essential ingredient in the calculation of $\Delta$ is the stopping power $(dE/dx)$ of individual SM species as a result of interactions with stellar constituents.  
...
The stopping of nucleons and pions in a WD are calculated in Appendix \ref{sec:Appendix} and depicted in Figures \ref{fig:SPnuc} and \ref{fig:SPpion}.
For energies greater than the typical nuclear binding energy $E_\text{nuc} \sim 10 ~\text{MeV}$, the energy loss is dominated by nonelastic nuclear collisions in which an incoming hadron strongly interacts with a target nuclear constituent to produce several outgoing, high-energy hadrons.
...
If these secondary hadrons are also sufficiently energetic, the result will be a roughly collinear hadronic shower. 
Shower constituents will typically be nucleons and pions, and the energy loss contained in a hadronic shower is described by the radiative stopping power
\begin{equation}
  \l \frac{dE}{dx} \r \sim \frac{E}{l_\text{h,non}},
\end{equation}
where $l_\text{h,non} \sim 1/n_\text{ion} \sigma_\text{h,non}$ is the characteristic mean free path for nonelastic collisions with nuclei in the WD.
The shower terminates when constituents reach the critical energy $\sim 10 ~\text{MeV}$, so that the shower length induced by an initial hadron of energy $\epsilon$ is approximately 
\begin{align}
\label{eq:hadlength}
  X_{\text{had}} \sim l_\text{h,non} \log\l\frac{\epsilon}{E_\text{nuc}}\r
  \approx 10 ~l_\text{h,non}.
\end{align}
At this stage, protons and neutrons lose energy predominantly through elastic nuclear scatters, characterized by the typical mean free path for elastic nuclear scattering $ l_\text{h,el} \sim 1/n_\text{ion} \sigma_\text{h,el}$. 
Thermalization of ions will occur after these nucleons have transferred $\OO(1)$ of their energy to nuclear targets in the form of a random-walk, which occurs after $\sim 10$ collisions. 
On the other hand, for charged pions we find the energy is predominantly transferred to electrons in the stellar medium via Coulomb collisions.
In this case, the desired length scale depends on the distance over which electrons below $\sim 10~\MeV$ are able to thermalize with ions. 
...
This is to be expected since these electrons can acquire a temperature at most just above the Fermi energy and therefore disperse their energy over a large region. 
Therefore, since the initial energy is split roughly equally among final-state hadrons, we simply ignore the contributions from charged pions in the hadronic shower and restrict our attention to final-state nucleons. 
...
If many low-energy hadrons are released and thermalize collectively, they will produce an ion profile centered on their origin with size $\Delta \approx X_\text{h} + \lambda_\text{nuc,el}$.
...
This is aimed towards calculating the stopping powers of high-energy electrons, photons, and light hadrons (protons, neutrons, pions) within the stellar medium, the results of which are summarized in Figures \ref{fig:SPelectron}, \ref{fig:SPphoton}, \ref{fig:SPnuc}, and \ref{fig:SPpion}.

We assume a carbon-oxygen WD, which is composed of fully-ionized carbon ions and a degenerate gas of electrons.
The stellar medium is very dense, with electron and ion number densities of $n_e = Z n_\text{ion} \sim 10^{30} - 10^{33} ~\cm^{-3}$, where $Z = 6$ is the charge of a carbon ion. 
This large density provides efficient stopping of incident particles, while also giving rise to qualitatively different stopping behavior than seen in terrestrial detectors. 
...
\end{equation}
which is much larger than the typical stellar temperature $T \sim \keV$ \textcolor{blue}{check the range of this temperature in the WD population}.
...
\label{eq:lattice}
  \omega \sim \frac{Z^2 \alpha}{n_\text{ion}^{-1/3}} 
         \sim 10^{-2} - 10^{-1} ~\MeV
\end{equation}
which typically stronger than the temperature, so the ions form a Coulomb lattice with lattice energy $\omega$ \cite{Teukolsky}. 
...
A particle of mass $m$, charge $e$, and velocity $v$ incident on a stationary target of mass $M$ and charge $Ze$ will transfer an energy $W$ as given by the recoilless Mott cross-section 
\begin{equation}
  \frac{d \sigma}{dW} = \frac{2 \pi \alpha^2 Z^2}{M v^2} \frac{1}{W^2} 
    \l 1-v^2 \frac{W}{W_\text{kin}} \r,
\label{eq:mott}
\end{equation}
where $W_\text{kin}$ is the maximum possible energy transfer satisfying kinematic constraints (i.e., a backward scatter),
\begin{equation}
\label{eq:ekin}
  W_{\text{kin}} = \frac{2 M v^2 \gamma^2}{1+ 2\gamma M/m +(M/m)^2},
\end{equation}
...
The integration limits are set by the maximum energy transfer $W_\text{kin}$, and by a minimum energy transfer $W_\text{min}$ which may be set by a variety of effects discussed below. 

We first compute the energy loss for scattering off carbon nuclei.
In this case, $n_\text{target} = n_\text{ion}$ and the stopping power is parametrically of order
\begin{align}
\label{eq:SP}
  - \l \frac{dE}{dx}\r \sim \frac{n_\text{ion} Z^2 \alpha^2}{M v^2} 
...
\end{equation}
which imposes an effective maximum impact parameter for Coulomb scatters. 
This gives a minimum energy transfer of 
\begin{equation}
\label{eq:W-TF}
  W_\text{TF} \sim \frac{Z^3 \alpha^3 n_e^{2/3}}{v^2 M},
\end{equation}
where we have used the fact that a soft Coulomb scatter at impact parameter $b$ will transfer an energy 
\begin{equation}
\label{eq:impact}
  W \sim \frac{Z^2 \alpha^2}{b^2 v^2 M}.
\end{equation}
...
We thus take the lower bound for energy transfer via free-particle Coulomb scattering off nuclear targets to be $\text{Max}\l W_\text{TF},~\omega\r$.
...
The cross-section \eqref{eq:mott} is no longer fully valid, as it ought to account for the significant target recoil and identical particle effects (i.e., the M{\"o}ller cross-section), as well as average over the relativistic motion of the target electrons. 
However, after integrating to find the stopping power, these are only $\OO(1)$ corrections. 
