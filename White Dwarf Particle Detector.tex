...
An incident particle of mass $m$, charge $e$, and velocity $\beta$ scattering off a (stationary) target of mass $M$, charge $Ze$ is described by the Rutherford differential cross section
\begin{equation}
\label{eq:rutherford}
...
\end{equation}
where $\gamma = (1-\beta^2)^{-1/2}$ is the relativistic factor. Quantum mechanical uncertainty sets a limit to the accuracy that can be achieved in ``aiming" an incident particle at a target. In terms of the collisional impact parameter $b$, this results in a bound $b > \frac{1}{\text{min}\{{M, m}\} \beta \gamma}$ and in terms of energy transfer, this translates to the condition
\begin{equation}
