...
\textcolor{blue}{Why don't these plots have the elastic nuclear scattering lines?}
In terrestrial detectors at the energies, the stopping powers would be dominated by Coulomb scattering of target electrons, however in the WD these collisions are suppressed by degeneracy of the target electrons and their efficient charge screening.
...
If these secondary hadrons have energy above $E_N$, they too are predominantly stopped by inelastic nuclear collisions and continue the processes, giving a roughly collinear shower. 
The hadrons in the shower are loosing $\OO(1)$ of their energy at each traversal of the mean free path for nuclear collisions $l_\text{had}$, so the shower constituent energy is described by a stopping power:
\begin{equation}
  \l \frac{dE}{dx} \r \sim \frac{E}{l_\text{had}}. 
\end{equation}
The shower terminates when the constituents reach engery $E_N$. 
For an initial particle energy $\epsilon$, this gives a hadronic shower length
\begin{align}
\label{eq:hadlength}
  X_{\text{had}} \sim l_\text{had} \log\l\frac{\epsilon}{E_c}\r
  \approx 10 ~l_\text{had},
\end{align}
where we have approximated the logarithmic energy dependence as an $\sim 10$ constant

After traveling a distance $\sim X_\text{had}$, the initial hadron energy is contained in an exponential number of final-state nucleons and pions of energy $\sim E_N \sim 10 ~\text{MeV}$.
At this stage, these hadrons will predominantly transfer energy to nuclei via nuclear elastic scatters. \textcolor{blue}{the above plots show coulomb scatters as dominating in this regime}
This ends the energy deposition by thermalizing the ions near the shower end-point. 
During this process, the hadrons execute a random walk covering a distance $X_\text{elastic} \sim l_\text{el} \sqrt{N}$, where $ l_\text{el}$ is the mean free path for elastic nuclear scattering and $N$ is the number of scatters required for thermalization.

In the above language, for a single hadron $R_\text{hadron} \sim 10 ~\l_\text{had} \sim [] ~\text{cm}$ and $X_\text{hadron} \sim () ~\l_\text{el} \sim [] ~\text{cm}$.  

% For the neutrons and protons, roughly $10$ collisions are needed to transfer the energy of these hadrons to ions, while for the less-massive pions roughly $100$ collisions are required.
