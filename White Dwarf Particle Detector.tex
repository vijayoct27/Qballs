...
This amounts to understanding the heating length $L_0$ and energy deposit $\Ez$ due to a given DM encounter with a WD - if these satisfy condition \eqref{eq:boom}, then the encounter is explosive.
...
In particular, any DM candidate which couples to the SM will generically be able to release SM secondaries in the star, which then transfer their energy to the stellar medium. 
...
It is therefore necessary to understand how energy is transferred from SM particles to the stellar medium in order to assess the explosiveness of these encounters, and for this reason the remainder of this section is dedicated to computing the heating properties of SM particles.

We will consider a primitive process that releases $N$ SM particles of species $i$, each with energy $\epsilon$. 
We begin by considering the heating due to a single released SM particle of energy $\epsilon$ and species $i$, which is most accurately described by the shape of the resulting temperature profile.
We parameterize this profile by two length scales: the displacement $R_i(\epsilon)$ of the center of the profile from the starting location of the SM particle, and the width $\Delta_i(\epsilon)$ of the temperature peak.  
Note that $R_i(\epsilon)$ may be much larger than $\Delta_i(\epsilon)$, but at its smallest $R_i(\epsilon) \sim \Delta_i(\epsilon)$.
We identify these two length scales as either of them may be relevant in determining the heating length $L_0$ of a general DM-WD encounter, depending on their relative sizes and on the number of SM particles involved.  
For example, if in an encounter a single high-energy SM secondary is produced, then $L_0 \sim \Delta_i(\epsilon)$ as the displacement of the heat from the SM production event is irrelevant. 
