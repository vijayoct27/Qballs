...
The trigger size is a measure of the WD's ability to quench runaway fusion, which is depends on stellar density and is set by the thermal diffusivity of photons or degenerate electrons \cite{Woosley}.
The diffusive cooling timescale of a region increases with the region's size, whereas the fusion heating timescale is independent of size.
Thus, there is also a threshold length scale such that a heated region with $T \geq T_f$ and sufficient linear size will eventually be dominated by fusion and result in a runaway chain-reaction.  
The trigger size $\lambda_T$ is the threshold for a minimally heated fusing region, $T = T_f$. 
For hotter regions, $T > T_f$, the threshold decreases slightly with temperature,
\begin{equation}
\label{eq:runaway}
  R \gtrsim \lambda_T \l \frac{T_f}{T} \r^{1/3}, 
  ~~~~ T \gtrsim T_f
\end{equation}
