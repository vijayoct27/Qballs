...
For energies greater than the typical nuclear binding energy $E_\text{nuc} \sim 10 ~\text{MeV}$, the energy loss for all hadrons is dominated by nonelastic nuclear collisions. 
These collisions produce a lighter, excited nucleus and a few outgoing hadrons which roughly carry away the incident particle's kinetic energy in equal fractions. \textcolor{blue}{citation}
...
As each collision divides the incoming energy equally among a few products, the energy loss of shower constituents is described by the radiative stopping power
\begin{equation}
  \l \frac{dE}{dx} \r \sim \frac{E}{l_\text{h,inel}},
\end{equation}
where $l_\text{h,non}$ is the mean free path for nonelastic hadron-nuclei collisions.
The shower terminates when the constituents reach engery $E_\text{nuc}$, giving a shower length of 
\begin{align}
\label{eq:hadlength}
  X_{\text{h}} \sim l_\text{h,inel} \log\l\frac{\epsilon}{E_\text{nuc}}\r
  \approx 10 ~l_\text{h,inel},
\end{align}
for an initial hadron energy of $\epsilon$. 

The shower terminates in a small cloud of final state nucleons and pions, each with energy $E_\text{nuc}$. 
