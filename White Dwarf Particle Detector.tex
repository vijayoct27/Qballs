...
Carbon ions themselves rapidly lose energy to cold electrons via Coulomb interactions, which in turn brem photons that thermalize via Compton scatters.
The range of ions is given in \textcolor{blue}{figure} and that of electron bremsstrahlung and Compton scattering in \textcolor{blue}{figure} - these are all below the trigger size at energies $~\MeV$.
...
However, after the ions warm the degenerate electron gas to temperature $T$, there will be a thermal population of electrons carrying energy $E \approx E_F + T$ that are able to brem up to $k\sim T$ and thus thermalize photons. 

We therefore see that any processes which heats ions also establishs a hot EM bath.
...
As shown in \textcolor{blue}{figure}, electron stopping at low energy is dominated by bremsstrahlung and photon stopping by Compton scatters.
...
The details of this evolution depend on the initial EM gas temperature, which is set by the total SM energy releases by the DM.
...
This heating will extend over the scale $X_{\gamma-\text{nuc}}$ needed to begin photonuclear showers, which is indeed below the trigger size.
...
Note that for the most dense WDs, the electron Fermi energy is $E_F \sim 10~\MeV$ - at these temperatures the electrons are degenerate and heating proceeds via the thermal tail with energies $\approx E_F + T$.
This is an essential distinction, as the degenerate electron range diverges at $E_F$ due to Pauli-blocking.
This range is plotted in \textcolor{blue}{figure}, where it is seen that for EM temperatures $1-10~\MeV$ the electrons thermalize over a distance of order the trigger size.
...
For the case of incident electrons, the low-energy stopping power includes an additional restriction that the incident electron not fall into the Fermi sea after transferring energy. We approximate this effect by simply setting $dE / d x = 0$ for incident electrons with energies $E\lesssim E_F$ the Fermi energy. For incident electrons with energies $E\gtrsim E_F$, the electrons are relativistic and the above results gives a good approximation. 
...
Inserting equation~\eqref{eq:CoulombRelativisticApprox},
\begin{align}  
