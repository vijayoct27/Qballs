...
In order for a collision or decay to trigger runaway fusion, we demand that it release sufficient energy \eqref{eq:coldecay}. This is possible to achieve in a number of ways. For instance, 
...
\end{align}
In other words, if \eqref{eq:transitmass} were violated then the DM interaction is either not strong enough to ignite the WD or it is so strong that it has no chance to penetrate the crust and transit the interior. This translates to a condition that the DM mass must be greater than $\sim 10^{29} ~\GeV$, although this bound is only applicable when the energy input to the WD is chiefly coming from the DM kinetic energy rather than binding energy or other sources. 
