...
\]
Since the DM is non-relativistic, $\mathcal{E} \lesssim 2 m_{DM}$ and this process is only relevant for $m_{DM} \grtsim 10^{14} \, \GeV$.


\subsection{DM Transit}

A DM transit releases energy continually along the DM particle's trajectory, resulting in a cylindrical temperature profile with some transverse length scale $L$.  This $L$ is determined by the DM-SM interaction parameters and $n$.  Ignition will be determined by both $L$ and the amount of energy deposited per length along the DM trajectory, so we parameterize this interaction in terms of a normalized linear energy deposit $\chi_{led}$:
\[
    \chi_{led} = 
    \frac{1}{n} \frac{\partial{E_{\text{medium}}}}{\partial{x}}
\]
where $n$ is the number density of the stellar medium and $\partial{E}/\partial{x}$ is the kinetic energy deposited to the stellar medium per DM distance traveled.  Nominally, $\chi_{led}$ will depend on the stellar density $n$, the instantaneous DM energy $E$, and the details of the DM-SM interaction.  However, over the range of WD densities we consider it is reasonable that the only variation in $\partial E /\partial x$ for different stars is a linear scaling in $n$, so $\chi_{led}$ is independent of stellar variations.  \textcolor{blue}{does this need more justification?}  Further, all DM particles will enter the WD with speeds of order the escape velocity of the star, which is roughly a constant $v_{esc} \approx 10^{-2}$ for all WDs.  Thus the initial energy of a transiting DM particle depends only on its mass and so $\chi_{led}$ for a transit event is a function of only DM properties: $m_{DM}$ and its DM-SM interactions.  

In order to ignite a WD during transit, the energy released must exceed the threshold \eqref{en:boom}.  If $L > \lambda_T$, then we study one segment $L$ of the transit which behaves as a single energy deposit of length-scale $L$, as described in Section \ref{sef:review}. If $L < \lambda_T$, then over a $\lambda_T$ segment of the transit we have many $L$-scale heating events which sum to give a total energy deposited over the scale $\lambda_T$.  We consider thus the energy deposited in a transit of length $\text{Max} \left[ \lambda_T, L \right]$, and requiring this to be greater than the threshold \eqref{eqn:boom} we find a lower bound on the $\chi_{led}$ sufficient to trigger runaway fusion:
\begin{equation}
...
\begin{equation}
\alpha\left(T_f\right) \lesssim v_\text{esc} \text{max}\left[\lambda_T, L\right]
\end{equation}
This condition is independent of DM model, and is satisfied for all WD densities. 
\textcolor{red}{Show this is actually true for all densities.} 

\textcolor{blue}{Similarly, we also require that the time to transfer energy $\epsilon$ out to the initial temperature profile of length $L$ is less than the diffusion time scale $\tau_d$.} \textcolor{red}{I'm not so sure this is really a condition - I this time scale is a sort of 'constant offset' time between the arrival of the DM and appearance of a temp profile $L$, and its size doesn't effect whether or not the energy adds. Let's discuss this.}
...
\[
    \chi_{sp} = 
    - \frac{1}{n} \frac{\partial{E_{\text{DM}}}}{\partial{x}}
\]
For a crust size $R_{\text{c}}$ \textcolor{blue}{$\sim 1\, \text{km}$} we require:
\begin{equation}
\frac{\chi_{sp}}{m_{DM}} \lesssim \frac{v_{esc}^2}{n R_c}
\end{equation}
Note that $\chi_{led}$ and $\chi_{sp}$ are not necessarily equal, though they will be in the important special case that the DM-SM interaction converts the only DM kinetic energy into relativistic SM particles. We will consider the special case below, and also the explicit model of Q-ball dark matter in which $\chi_{led} >> \chi_{sp}$ . 

It is important to note that if either of these condition are violated it is still possible to have an exploive transit event, but the formalism of equation \eqref{eq:transitexplosion} is no longer valid and a more complicated explosion condition is needed.  \textcolor{blue}{For simplicity, in this work we will focus on events that satasify the above, but the generalize to other events is straight-forward. Sketch this generalization?}

{\color{red}
I am not sure the crust condition is a strict veto of explosion events. What happens if a DM is 'stopped' by the crust?  It will thermalize with the stellar medium, but it's velocity will still typically be much smaller than that of the medium, so it will drift towards the center.  There will be some biased random-walk of a thermal DM to the center - can this ignite fusion along the way?  If so, we should comment and then perhaps ignore it as too complicated to be worth working out if we dont have an explicit model in mind.  But I'm also not sure if this could ever ignite fusion anyway... Let's discuss this.}
...
>>>>>>> 1275e8ad79d67b45a2a8f68bf04821da5998bdee
