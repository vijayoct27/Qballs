...
The Klein-Nishina cross section (integrated over all scattering angles) evaluated in the electron rest-frame is given by
\begin{equation}
\sigma_C \approx \frac{\pi \alpha^2}{2 m_e k} \left \{1+\log{\l\frac{4 k^2}{m_e^2} \r}\right \}, ~~~ k \gg m_e,
\end{equation}
Since the electrons in a WD are sufficiently relativistic, the Thomson scattering limit is never applicable. In addition, we ignore the effects of degeneracy when the incident photon transfers is highly energetic such that scatter is sufficient to knock electrons out of the Fermi sea. 
...
Nuclear interactions can be either elastic or nonelastic collisions. Elastic collisions could result in an incident particle losing virtually all of its energy in a single (head-on) scatter if the incident and target masses are of the same order. However, in this section we will be primarily concerned with light hadrons (i.e. protons, neutrons, pions) incident on nuclei of atomic number $A \sim 10$ as relevant for the WD. An elastic collision between an incident (non-relativistic) mass $m$ and a heavy, stationary target mass $M$ results in a fractional energy transfer
\begin{equation}
\frac{E_f}{E_0} \approx 1 - \frac{2m}{M}(1+ \cos \theta), ~~~ m \ll M. 
\end{equation} 
where $\theta$ is the scattered angle of the incident particle. Of course, the deposited energy will be maximal for forward scatters $\theta = 0$ and vanish as $\theta \to \pi$. We take this to be the fractional energy transfer.



 where $m$ and $M$ are the masses of the incident hadron and nuclei respectively. However, since the ions in a WD form a Coulomb lattice with binding energy \eqref{eq:lattice}, elastic collisions cannot transfer energy less than $\sim E_B$. This sets a lower bound on the energy loss function for elastic scatters, which is of the form
...
For the purpose of energy deposition, 
