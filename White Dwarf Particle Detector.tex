...
In order to adequately place constraints on a DM transit, it is necessary to make assumptions regarding general properties of the DM-WD interaction. 
These conditions are most naturally posed in terms of the mass along with the dark matter stopping power $\partial E_\text{DM} / \partial x$ and the LET rate $\partial E_\text{SM} / \partial x$. To make contact with more familiar constraint plots, we would like to place constraints in terms of a dark matter cross section versus mass, so let us now define this cross section and consider its connection to the conditions.

Let $\sigma_{i,\epsilon}^j$ be the DM-SM cross section to produce $N_{i,\epsilon}$ particles of species $i$ and individual energy $\epsilon$ via the interaction of DM with species $j$ in the star. Then we can relate the LET rate to the cross section according to
\begin{align}
