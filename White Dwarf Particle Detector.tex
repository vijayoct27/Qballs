...
If the DM then interacts to release $N$ particles of typical energy $\epsilon$, we require $\epsilon \gtrsim T_f$ and $N \epsilon \gtrsim \Eboom$ as these are necessary for ignition.
We are thus sensitive to a GUT-mass or heavier DM particle, likely a composite state, which interacts in the WD to produce either a few GUT-scale SM particles or a very large number of collier-scale particles. 
...
The heating due to heavier species is straightforward to determine following the approach used below.
...
With the above picture in mind, we now quantify the SM heating by considering $N$ particles of SM species $i$ and uniform energy $\epsilon$ released isotropically from a point in the WD interior, and compute the width $\Delta_i(N, \epsilon)$ of the resulting ion temperature peak. 
If the DM encounter indeed has this form, then the heating length is given simply by $L_0 \sim \Delta$. 
However, in a more realistic interaction the SM secondaries will be produced according to some distribution in energy and species, and one can find $L_0$ by an appropriate average over primitive $\Delta$s computed here. 
...
Finally, an essential ingredient in this calculation is the stopping power $dE/dx$ of SM species due to various components of the stellar medium. 
...
The lambda of various species are plotted in Figures \ref{fig:SOMETHING}, with the dominant stopping mechanisms indicated. 
We will summarize here the salient features of these dominant stopping mechanisms, with a more detailed analysis of these stopping powers is provided in Appendix \ref{sec:appendix}. 
% add here figures showing dominant ranges (or stopping powers) for each species, with dominant mechanism annotated.  Maybe add the e-ion subdomiant processes to that plot, as those are important. 

\subsection{Heating Properties of Hadrons}

First consider the release of low-energy light hadrons, $\epsilon \lesssim 10~\MeV$.
