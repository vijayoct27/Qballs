...
\end{align}
where $R_\text{cr} \sim \text{km}$ \textcolor{red}{check this} is the width of the non-degenerate WD crust, and $v_\text{esc}$ is the escape velocity of the WD.  In addition we have conservatively assumed that the crust density is $\OO(1)$ of the interior density. \eqref{eq:CrustCondition} is an important condition to satisfy as it is only in the degenerate WD interior that runaway fusion can occur.  Were this condition not assumed, we would have to account for the possible variations in the kinematic state of the DM after traversing the crust for various stopping powers. Furthermore, since $R_\text{cr}$ will always be larger than the eventual heating lengths $L_0$, this condition ensures that there is negligible variation in a DM trajectory through a heating region. 

The ability of a transit to ignite the star is most generally parameterized by a \emph{linear energy transfer} (LET) $d E_\text{SM}/d x$, which is the energy released into thermalizing SM products per distance traveled. Note that while the LET and stopping power are similar in nature, these model-dependent parameters are not necessarily related. Although these are equal in the sensible special case that the DM-SM interaction converts only DM kinetic energy into relativistic SM particles, if most of the released energy is due to changes in mass, then it is possible for these two quantities to be very different.
