...
\begin{equation}
v_\text{th} \sim \sqrt{\frac{T}{m_\text{DM}}},
\end{equation}
...
&\approx 10^{-3} ~\text{m} \l \frac{10^{16} ~\GeV}{m_\text{DM}}\r^{1/2} \l \frac{10^{32} ~\cm^{-3}}{n_\text{ion}}\r^{1/2}, \nonumber
\end{align}
...
To find the time necessary to accumulate this number of DM states, we first write down the DM transit rate through the WD
\begin{align}
\Gamma_\text{transit} \sim \frac{\rho_{\text{DM}}}{m_\text{DM}} R_\text{WD}^2 \l\frac{v_\text{esc}}{v}\r v_\text{esc}.
\label{eq:TransitFluxCondition}
\end{align}
As a result, the amount of time $t_\text{col}$ required to capture sufficient DM to induce self-gravitational collapse is 
\begin{align}
\label{eq:tcol}
t_\text{col} &\sim \l \frac{\rho_\text{WD} R_\text{th}^3}{m_\text{DM} \Gamma_\text{transit}} \r \\
&\approx 10 ~\text{s} \l \frac{10^{16} ~\GeV}{m_\text{DM}} \r^{3/2} \l \frac{10^{32} ~\cm^{-3}}{n_\text{ion}}\r^{1/2}.
\end{align}
As a comparison, the unstopped DM wind will spend roughly $R_\text{WD}/v_\text{esc} \approx 0.1 ~\text{s}$ in the WD.
However, there is a caveat - if $m_\text{DM}$ is too large, then there is not enough time for the DM to even collect at the thermal radius once it is captured. 
Assuming a swift thermalization, this collection time is $R_\text{WD}/v_\text{th}$ and is less than the lifetime of the WD DM masses $m_\text{DM} \lesssim 10^{32} ~\GeV$. 
Once the DM has collected at the thermal radius, it will collapse into a core after a time \eqref{eq:tcol}.
...
\label{eq:taugamma}
\end{equation}
Ultimately if the process is such that the explosion condition $\eqref{eq:coldecay}$ is satisfied, then we can place constraints by comparing \eqref{eq:collisionDM} and \eqref{eq:taugamma} to either the WD lifetime or the SN rate in an appropriate fashion. 
A similar analysis can be done in the case of DM capture. 
The dominant constraint on $\sigma_\text{DM-DM}$ comes from demanding that the self-gravitating DM collected at the thermal radius does not have an annihilation event during the timescale of the collapse. 
...
\begin{equation}
\frac{(\Gamma_\text{transit} t_\text{col})^2 \sigma_\text{DM-DM}}{R_\text{sta}^2} \lesssim 1,
\end{equation}
where $(\Gamma_\text{transit} t_\text{col})$ is the total number of DM particles in the collapsing sphere.
...
In addition, since the DM number density increases as it gets closer to the stabilized radius $R_\text{sta}$, one can show that in this case that the annihilation rate becomes 
\begin{equation}
\Gamma_\text{col} \sim \frac{\Gamma_\text{transit}^2 \sigma_\text{DM-DM}}{v_\text{th} R_\text{sta}}.
\end{equation}
...
As we have not seen any cosmic rays of energy greater than $\sim 10^{12} ~\GeV$ \cite{ThePierreAuger:2015rha, AbuZayyad:2012ru}, this places a concrete limit on the DM interaction parameters $\sigma_\text{DM-DM}$ and $\tau_\text{DM}$ when such ultra-high energy particles are released. 
...
Note that a similar bound on DM self-interactions $(\sigma_\text{DM-DM}/m_\text{DM})$ due to colliding clusters of galaxies \cite{Randall:2007ph} is of the same order of magnitude as that derived considering our own galaxy.
...
\end{align}
For the typical parameters of a $1.25 ~M_{\odot}$ WD we find that the DM mass must be greater than $\sim 10^{29} ~\GeV$ to ensure a bullet-like and explosive transit, taking the density of the WD crust $\rho_\text{crust}$ to be a nominal $\OO(10^{-2})$ fraction of the central density $\rho_\text{central}$ \cite{Chandrasekhar}.
