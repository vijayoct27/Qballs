...
\end{align}
where $l_\text{inel}$ is the mean free path for nuclear scatters, set by the cross-section $\sigma_\text{inel} \approx 100 ~\text{mb}$ and we have suppressed the weak logarithmic energy dependence. \textcolor{blue}{cite something}
...
\label{eq:photonuclength}
  X_{\gamma\text{-nuc}} \sim \frac{l_\text{inel}}{\alpha}
...
Neutrons and neutral pions are the simplest species we consider, interacting a these scales only via elastic nuclear scatters.
These are stronger than the inelastic interactions, $\sigma_\text{el} \approx 1 ~\text{b}$ \textcolor{blue}{cite something}.
In addition, the mass hierarchy between these particles and ions will require $\sim 10 - 100$ scatters to transfer the hadron's energy, further suppressing the range by a random-walk factor,
\begin{align}
 \lambda_\text{el} &\approx
 10^{-7} ~\text{cm} \l\frac{10^{32}~\text{cm}^{-3}}{n_\text{ion}}\r.
\end{align}
This is always less than the trigger size, and also smaller than the $\pi^0$ decay length $\delta_\pi \sim 10^{-6} ~\text{cm}$, so neutrons and neutral pions provide efficient heating.
