...
%Thus we are generally sensitive to ultra-heavy $>10^{15} ~\GeV$ DM particles, most likely composite states, which interact in the WD to produce either a few ultra-high energy particles or many lower energy particles. 
%Most typically, the lightest SM particle species will be released. 

The remainder of this Section is dedicated to computing the heating properties of SM species in a manner that is independent of the precise DM encounter. 
...
The heating length of any such deposition is ultimately determined by the distances individual particles travel before giving up $\OO(1)$ of their energy.
For every possible interaction of a given species described by a stopping power $(dE/dx)$, we define a length scale 
\begin{equation}
\lambda = \int_{\epsilon/2}^{\epsilon} \l \frac{dE}{dx}\r^{-1}E ~dE,
\end{equation}
which is the characteristic range over which that species would deposit its energy to either stellar constituents or to additional secondaries via this interaction. 
...
Subsequently, the resulting width of the initial ion temperature profile $L_0$ is computed for this schematic heating event for the specific case of light hadrons, electrons, photons, and neutrinos.
Note that the heating properties of heavier SM states can be calculated in a similar manner, although we do not do so in this work. 
...
>>>>>>> tinkering with SM Heating section
