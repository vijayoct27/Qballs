...
\label{sec:Review}
\textcolor{red}{I think the title here needs a connecting word, like 'Runaway Fusion in White Dwarfs'}
...
\label{sec:DMexplode}
\textcolor{red}{Not sure I like the word 'Explosiveness', would prefer something more descriptive like 'Dark Matter Induced Ignition' or 'Runaway Fusion via Dark Matter Heating'}
...
The stopping powers for incident nucleons (protons, neutrons) and pions in a WD are shown in Figures \ref{fig:SPnuc} and \ref{fig:SPpion}, respectively. Note that the depicted energy loss due to Coulomb collisions is only applicable for incident charged particles - the electromagnetic couplings of neutral hadrons are suppressed by higher-dimension operators. 
\begin{figure}
\label{fig:SPnuc}
\includegraphics[scale=.45]{SPnucleon.pdf}
\caption{Nucleon energy loss in a WD density $n_e = 10^{32} ~\text{cm}^{-3}$.} 
\end{figure}

\begin{figure}
\label{fig:SPpion}
\includegraphics[scale=.45]{SPpion.pdf}
\caption{Pion energy loss in a WD density $n_e = 10^{32} ~\text{cm}^{-3}$.}
\end{figure}
For incident energies greater than the typical nuclear binding energy $\epsilon \gtrsim 10 ~\text{MeV}$, the energy loss is dominated by nonelastic nuclear collisions in which secondary hadrons carry the initial energy in a roughly collinear shower. Although neutrons and $\pi^\pm$ have characteristically long lifetimes, the mean distance traversed by a neutral pion before decaying to photons $d_{\pi^0} = \gamma v \tau \approx 10^{-6} ~\text{cm}$ for $\epsilon \sim 10 ~\text{MeV}$. However in a WD density $n_e \sim 10^{32} ~\text{cm}^{-3}$, the nuclear mean free path for a nonelastic collision is roughly $l_\text{had} \approx 10^{-7} \text{cm}$. Therefore, there will be minimal electromagnetic contributions due to $\pi^0$ decays during the progression of a hadronic cascade. \textcolor{blue}{nontrivial - not true in ATLAS}. Since this remains the dominant stopping power, the end products of any hadronic shower will consist of hadrons of kinetic energy $1-10 ~\text{MeV}$. 



As shown in Figure \ref{SPhad1}. In this model, the total hadronic shower length $X_{\text{had}}$ for an incident hadron of energy $E_0 \gtrsim 10 ~\text{MeV}$ is given by
\begin{equation}
