...
Note that the energy deposit required to trigger SN $\mathcal{E}_\text{boom}$ is, in the low-density regime, a strong function of WD density. In \cite{Graham:2015apa}, the WD central density was consistently used to describe primordial black hole transits. However, it is found that the WD density changes an $\OO(1)$ fraction from the central value out at a distance $\sim R_\text{WD}/2$ \textcolor{blue}{cite}. Therefore using the central density is a decent approximation as long as we consider heating events within this ``modified" WD volume, and from simplicity we use this approach. 
