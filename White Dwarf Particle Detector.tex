...
DM slows down in the WD to a velocity $\sim \sqrt{T/m_\text{DM}}$, where $T \sim \keV$ is the WD temperature. 
The captured DM will virialize and accumulate at the thermal radius 
\begin{equation}
R_\text{th} \sim \l \frac{T}{G m_\text{DM} \rho_\text{WD}}\r^{1/2} \approx 10^{-3} ~\text{m} \l \frac{10^{16} ~\GeV}{m_\text{DM}}\r^{1/2},
\end{equation}
where we have assumed the WD has roughly constant density $\rho_\text{WD}$ equal to the central value for a $\sim 1.25 ~M_{\odot}$ WD within a sphere of radius $R_\text{th}$. 
Gravitational collapse into a compact DM core begins once when the total mass of DM exceeds the WD mass within this region. 
To find the time necessary to accumulate this number of DM states, we use the transit rate for DM matter

as a comparison, the unstopped DM wind will spend roughly $v_\text{esc}/R_\text{WD} \approx 1 ~\text{s}$ in the WD.
...
We can compute the expected DM-DM collision rate parameterized by cross-section $\sigma_\text{DM-DM}$
\begin{equation}
\Gamma_\text{collision} \sim n_\text{DM}^2 \sigma_\text{DM-DM} v R_\text{WD}^3,
