...
To compute $\Delta$, we follow the energy transfer from the initial released particle into ions, possibly through several intermediary species, and identify the length scale over which the ions are thermalized.
An essential ingredient in this calculation is the stopping power $dE/dx$ of SM species with various components of the WD medium, and the associated range $\lambda \sim E \cdot (dE/dx)^{-1}$, which is to $\OO(1)$ the distance a particle travels before losing $\OO(1)$ of its energy.  
...
\label{eq:hadlength}
  ...
  \sim 10 ~l_\text{h,non} \sim
  10^{-6} ~\text{cm} \l\frac{10^{32}~\text{cm}^3}{n_\text{ion}}\r . 
\end{align}
...
\begin{equation}
  X_{\pi,\text{el}} \sim 10~l_\text{h,el} \sim 
  10^{-8} ~\text{cm} \l\frac{10^{32}~\text{cm}^3}{n_\text{ion}}\r
\end{equation}
