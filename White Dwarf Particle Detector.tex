...
An incident electron or charged  hadron will Coulomb scatter off carbon ions.
This stopping power is most easily understood if we describe these scatter in terms of an impact parameter $b$.
For an incident particle of mass $m$, charge $e$, and speed $\beta$, the energy transfered in a soft scatter is 
\begin{align}
\label{eq:omega_softscatter}
    \omega \approx \frac{e^4 Z^2}{2 m_c \beta^2 b^2}
\end{align}
where $Z$ is the ion charge number and $m_c$ the ion mass.  
This is valid only for sufficiently small $\omega$, and thus fails as $b \rightarrow 0$.
While \ref{eqn:omega_softscatter} diverges for vanishing $b$, there is in fact a maximal energy transfer allowed by kinematics, corresponding to an exactly backward scatter at $b=0$,
\begin{align}
  \omega_\text{kin} \approx \frac{2 p^2}{m_c}
\end{align}
where $p$ is the incident particle momentum. 
For small $b$, the energy transfer will saturate at $\omega_\text{kin}$. 
We can thus characterize the energy transfer as 
\begin{align}
    \omega \approx \begin{cases}
    \frac{2 p^2}{m_c},                 & \, b < b_0 \\
    \frac{e^4 Z^2}{2 m_c \beta^2 b^2}, & \, b > b_0
    \end{cases}
\end{align}
where $b_0 = e^2 Z / 4 \beta p$ is a characteristic transition scale between hard and soft scatters. 
...
The finite stopping power is 
\begin{align}
  \frac{dE}{dx} &= n_\text{ion} 
    \int_0^{\lambda_{TF}} \, 2\pi  \, db \cdot \omega \\ 
    & = 2 \pi n_\text{ion} \left( \omega_\text{kin} \int_0^{b_0} \, db 
    + \frac{e^4 Z^2}{2 m_c \beta^2} 
    \int_{b_0}^{\lambda_{TF}} \,\frac{db}{b} \right)
\end{align}

%  momentum $p$. 
% }
% The energy loss in these scatters  is easily under
% The ion is nearly at rest and heavier than the incident particles we consider, so this , so this scattering is given to a good approximation the recoilless Mott cross-section
% \begin{align}
%     \frac{d\sigma}{d\cos\theta} \approx \frac{e^4}{64\pi^2 \beta^2 p^2}
%     \frac{1}{\sin^4\left(\frac{\theta}{2}\right)} 
%     \left[1 - \beta^2 \sin^2\left(\frac{\theta}{2}\right) \right]    
% \end{align}
% provided that the incident particle is not too energetic.
% Here $\theta$ is the scattering angle, and $e$ is the charge, $\beta$ the velocity, and $p$ the momentum of the incident particle.
% Of course a small ion recoil does occur, which is responsible for a small energy loss per scatter, 
% \begin{align}
%     \omega \approx 2\frac{p^2}{m_c} \sin^2\left(\frac{\theta}{2}\right)
% \end{align}
% where $m_c$ is the carbon ion mass.
% These results are accurate so long as the ion recoil is non-relativistic, $p \lesssim m_c \sim 10~\GeV$. 
% For very high incident energies, a good approximation is now obtained by taking $m \approx 0$, for which
% \begin{align}
%     \frac{d\sigma}{d\cos\theta} &\approx \frac{e^4}{64\pi^2 p^2}
%     \frac{1}{\sin^4\left(\frac{\theta}{2}\right)} \cdot \frac{1}{2} \\
%     \omega &\approx p
% \end{align}
% \textcolor{blue}{add citation}

% The stopping power is given by an integral over all possible scatters
% \begin{align}
%     \frac{dE}{dx} = n_\text{ion} 
%         \int \; d\left(\cos\theta\right) 
%         \left(\frac{d\sigma}{d\cos\theta}\right) \omega.      
% \end{align}
% This is a nominally divergent quantity due to soft scatters ($\theta\rightarrow0$), corresponding to the infinite range of the Coulomb interaction. 
% In the WD medium, however, electron plasma screening cuts off soft scatters. 
% The screening length scale $\lambda_{TF}$ is given in the Thomas-Fermi approximation by \cite{Teukolsky}
% \begin{align}
% \label{eq:TF}
