...
DM with negligible energy loss in the WD medium will traverse the star in roughly a time $\sim R_\text{WD}/v_\text{esc} \approx 0.1 ~\text{s}$ and have a number density within the WD enhanced relative to the average galactic density by a factor $v_\text{esc}/v \sim \OO(10)$.
...
\end{equation}
Note that the DM need only lose velocity of order the galactic virial velocity to get captured. 
...
First, the time for thermalized DM to ``drift" down to the thermal radius is roughly
\begin{align}
\label{eq:tdrift}
  t_\text{drift} \sim \frac{R_\text{WD}}{v_\text{th}}
  \approx 50 ~\text{yr} ~ \l \frac{m_\text{DM}}{10^{16} ~\GeV} \r^{1/2}. 
\end{align}
Note that for DM masses greater than $\sim 10^{30}~\GeV$, a DM cloud cannot possibly form within the lifetime of the star.
The time $t_\text{collect}$ needed for the star to collect a critical core mass of DM \eqref{eq:mcore} is given by
\begin{align}
\label{eq:tcol}
