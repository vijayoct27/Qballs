...
Although such candidates cannot be fundamental particles, in the presence of self-interactions it is natural to expect that the DM is able to form large composite states just as the SM produces complex structure of mass parametrically greater than the weak scale. 
...
The precise behavior is determined by the stopping power parameter $(dE/dx)_\text{SP}$.
...
However, in the case of DM capture the number density of DM particles in the WD is dramatically increased due to the exceptionally long lifetime $\sim \text{Gyr}$ of the WD - as a comparison, the unstopped DM wind will spend roughly $v_\text{esc}/R_\text{WD} \sim 1 ~\text{s}$ in the WD.
...
%\begin{equation}
%\Gamma_\text{collision} \sim
 %\begin{cases}
    %n_\text{DM}^2 \sigma R_\text{WD}^3 v_\text{esc} & \l \frac{dE}{dx} \r_\text{SP} \ll \frac{m_\text{DM} v_\text{esc}^2}{R_\text{WD}} \\
   %n_\text{DM}^2 \sigma R_\text{WD}^3 \sqrt{\frac{T}{m_\text{DM}}} & \l \frac{dE}{dx} \r_\text{SP} \gtrsim \frac{m_\text{DM} v_\text{esc}^2}{R_\text{WD}}\\
  %\end{cases},
  %\label{eq:collisionDM}
  %\end{equation} 
...
With the above schematic for DM-DM collisions, we constrain the cross section $\sigma_\text{col}$ as a function of $m_\text{DM}$ using the different classes of observation available and for representative choices of $\eta$ and SM species $i$ released.
...
Consider an annihilation or decay which releases $N$ SM particle of species $i$.
These will deposit their energy and thermalize ions within a distance explicitly calculated in Section \ref{sec:SMHeating}. 
With the above schematic for DM-DM collisions and DM decays, we can place bounds on the cross-section $\sigma_\text{col}$ or lifetime $\tau_\text{DM}$ as a function of $m_\text{DM}$.

However, there are additional constraints on DM that interact in this manner, which we now summarize. 
For one, DM can annihilate or decay into ultra-high energy particles within our galactic halo and therefore contribute to the cosmic ray flux seen in terrestrial air shower detectors.
As we have not seen any cosmic rays with energy greater than $\sim 10^{12} ~\GeV$ \cite{ThePierreAuger:2015rha, AbuZayyad:2012ru}, this places a concrete limit on the DM interaction parameters $\sigma_\text{col}$ and $\tau_\text{DM}$ when such ultra-high energy particles are produced in the form of photons, electrons, or hadrons. 
In theory, a similar constraint can also be placed on lower-energy SM products which would provide an additional source for the measured cosmic ray flux, although such a detailed analysis is beyond the scope of this work. 
One can calculate the expected cosmic ray flux due to a collision or decay event in the galactic halo as follows: 
The expected flux of DM decay products on earth is given by 
\begin{equation}
\Gamma_\text{earth} \sim \int_{\text{halo}} \frac{\rho_\text{DM}(r)}{m_\text{DM}} \frac{R_\text{det}^2}{L^2} \frac{1}{\tau_\text{DM}},
\end{equation}
where $R_\text{det}^2 \sim (100 ~\text{km})^2$ denotes the typical size of a terrestrial cosmic ray detector and $L$ is the distance from the DM decay event and the earth. 
$\rho_\text{DM}$ is the expected density profile of DM throughout the galactic halo.
This integral can be calculated numerically using an NFW profile fit (as given by \cite{Nesti:2013uwa}) and the known distance from the earth to the center of the galaxy, although to a very good approximation we see that the result is the same if we simply set the galactic parameters to the average values $\rho_\text{DM} \sim 0.4 ~\GeV/\cm^3$ and $L \sim 50 ~\text{kpc}$. 
The constraint on DM is derived by requiring that the expected time for an event to strike earth $\Gamma_\text{earth}^{-1}$ is less than the lifetime of the detector $\sim 10 ~\text{year}$.
Curiously, we find that in the case of DM decays in the galaxy, the resulting constraints are similar in magnitude to the constraints due to the observation of a local WD. 
In fact, this coincidence can be seen by comparing the effective ``space-time volume" for the two detectors.
A cosmic ray detector sees events within a space-time volume $\sim (R_\text{det}^2 R_\text{halo} \times 10 ~\text{year})$ which is numerically of the same order as the WD space-time volume for decay events $\sim (R_\text{WD}^3 \times 10 \times 10^{9} ~\text{year})$, where the additional factor of $\sim 10$ is due to the gravitational Sommerfeld enhancement. 
Note that a similar calculation can be done in the case of DM-DM annihilations, although in this case a parametrically larger Sommerfeld enhancement $\sim 1000$ results in more stringent constraints from the WD observation. 


In addition, there are various cosmological bounds on DM-DM collisions. If the DM-DM annihilation cross section were sufficiently big, this would affect our local halo by an $\OO(1)$ factor. 
Furthermore, DM-DM annihilations into electrons and photons would cause ionization and therefore affect the measured CMB temperature profile and fluctuations. 
It is found that the lifetime of DM $\tau_\text{DM} \gtrsim 100 ~\text{Gyr}$ in order to not screw up the CMB. 
...
Here we have outlined the many ways in WDs can constrain DM candidates with.
If a WD is   
