...
\begin{align}
\frac{d E_\text{DM}}{d x} \lesssim \frac{m_\text{DM} v^2_\text{esc}}{R_\text{cr}},
\label{eq:CrustCondition}
\end{align}
where $R_\text{cr} \sim \text{km}$ \textcolor{red}{check this} is the width of the non-degenerate WD crust, and $v_\text{esc}$ is the escape velocity of the WD.  In addition we have conservatively assumed that the crust density is $\OO(1)$ of the interior density. It is crucial that a transiting DM penetrate the crust as it is only in the degenerate WD interior that runaway fusion can occur.  If \eqref{eq:CrustCondition} were violated, we would have to account for possible variations in the kinematic state of the DM after traversing the crust for various stopping powers. Furthermore, since $R_\text{cr}$ will always be larger than the eventual heating lengths $L_0$, this condition ensures that there is negligible variation in a DM trajectory through a heating region. 
...
We now summarize the explosiveness of a Q-ball transit. Note that there is negligible stopping power for the Q-ball to slow down in a WD, and as such we may ignore condition \eqref{eq:CrustCondition}. According to the notation of Section \ref{sec:Constraints}, the Q-ball transit is completely parameterized by an LET which takes the simple form
\begin{equation}
\frac{d E_\text{SM}}{d x} = \sigma_Q n_\text{ion} \cdot N_{\pi,\epsilon} \epsilon,
\end{equation}
where the Q-ball interaction with nuclei results in $N_{\pi,\epsilon} \sim \OO(10)$ pions released, each with typical kinetic energy $\epsilon \sim 500 ~\text{MeV}$. Numerous experiments have studied interactions of pions in this energy range incident upon complex nuclei targets such as carbon. It is found that there is roughly equal cross section of order $\OO (100 ~\text{mb})$ for a (neutral or charged) pion to either elastically scatter or become absorbed in a nonelastic scatter with no final state pion \cite{Pionnuclear}. This confirms the assumptions made about nuclear interactions in Appendix \ref{sec:appendix} toward deriving the energy loss for pions in Figure \ref{fig:SPpion}. As seen in Section \ref{sec:HeatingLength}, nonelastic collisions are most relevant for energy loss and will induce a hadronic shower. The corresponding heating length is simply $L_\pi \approx \text{few} \times 10^{-7} ~\text{cm}$ in WD density of $n_\text{ion} \sim 10^{32} ~\text{cm}^{-3}$. 

The resulting explosiveness of the Q-ball interaction is plotted in Figure \ref{fig:boomQball}. If the Q-ball cross section is related to its mass and baryonic charge as in \eqref{eq:Qballprop}, we find that
\begin{equation}
