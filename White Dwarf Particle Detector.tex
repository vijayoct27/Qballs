...
\caption{Minimum energy $\mathcal{E}_\text{boom}$ required to trigger SN as a function of WD mass, based on numerical results for $\lambda_T$ \cite{Woosley}.}
\label{fig:Eboom}
\end{figure}
...
Having reviewed the conditions necessary for an energy deposit to initiate runaway fusion in a WD, we turn our attention to DM triggers of SN. 
...
This was done in \cite{Graham:2015apa} for the trivial case of primordial black holes which transit a WD and locally heat the star through gravitational dynamical friction. 
...
For these types of processes, the unknown DM physics serves to determine the initial distribution in space, momentum, and species of the SM particles produced in the star, while the actual heating proceeds entirely though known SM interactions. 
...
For this reason, the remainder of this section is dedicated to the heating properties of individual SM particles of given energy $\epsilon$ and species $i$. 
...
An essential ingredient for calculating $R_i$ and $\Delta_i$ is the stopping power of high-energy SM particles in the WD medium. 
To this end, we summarize here as needed the dominant sources of energy loss for each species while a more detailed analysis is reserved for Appendix \ref{sec:appendix}. 
...
These result in a lighter, excited nucleus and a few outgoing hadrons which roughly carry away the incident particle's kinetic energy in equal fractions. \textcolor{blue}{cite}
If these secondary hadrons have energy above $E_\text{nuc}$, they too are predominantly stopped by nonelastic nuclear collisions, producing a roughly collinear shower. 
...
\end{equation}
where $l_\text{h,non}$ is the mean free path for nonelastic hadron-nuclei collisions.
...
The hadronic shower necessarily terminates in a cloud of final state nucleons and pions, each with energy $\sim E_\text{nuc}$. 
The energy loss of these particles is dominated by elastic nuclear scatters,which represents the end-stage of the ion-heating process with thermalization occurring after the hadrons have scattered $\OO(1)$ of their energy.  
...
Over the next two sections, we parameterize aspects of the DM-induced heating of WDs that are purely dependent on the unknown physics of DM. 
In this Section, we derive explosion conditions on DM-WD encounters in terms of their heating lengths $L_0$ that determine whether or not they are capable of igniting the star.
...
In Section \ref{sec:Constraints}, we constrain generic models of DM which will trigger SN via one of these processes.
In order to do so, we additionally assume a simple, schematic form for the DM interactions such that $L_0$ can be calculated exactly using the results of Section \ref{sec:SMHeating} \textcolor{blue}{or table summary?}. 
...
We parametrize the DM kinetic energy loss per distance travelled in a WD by a DM stopping power $(dE/dx)_\text{SP}$. 
Consider the special case of a ``bullet-like" transit in which DM penetrates the non-degenerate crust of a WD with negligible change in kinetic energy
\begin{align}
\label{eq:CrustCondition}
  \left( \frac{d E}{d x} \right)_\text{SP} \ll 
  \frac{m_\text{DM} v^2_\text{esc}}{R_\text{crust}},
\end{align}
where $R_\text{crust} \sim 50 ~\text{km}$ is the width of a WD crust and $v_\text{esc} \sim 10^{-2}$ is the escape velocity of the WD. 
Typically, the density of the WD crust is a nominal $\OO(10^{-2})$ fraction of the central density \textcolor{blue}{cite}. 
If \eqref{eq:CrustCondition} were not satisfied, then the kinematics of a transit would be highly dependent on details of the DM stopping power and must be calculated from a specific model.
In addition, $R_\text{crust}$ is parametrically larger than any of the heating lengths considered in this work. 
Therefore, the assumption of a DM transit at roughly constant velocity also allows us to ignore DM energy loss within the degenerate WD interior, where runaway fusion can be triggered. 

The ability for a transit to ignite a star can be parameterized by a linear energy transfer $(dE/dx)_\text{LET}$, which is the energy released into SM products per distance traveled.
Note that while the LET and DM stopping power are conceptually related quantities and may be equal in important special cases, they are generically different.
...
Following the discussion of \cite{Graham:2015apa}, the constraints we place on DM candidates come from (1)~the existence of heavy, long-lived white dwarfs and (2)~the measured type Ia SN rate. 
...
Here we focus on a simple schematic where an annihilation/decay releases a number of SM particles $N_i$ of single species $i$ and energy $\epsilon$ in a point-like event.
Each particle will deposit its energy and thermalize ions within a heating length $L_0$, which was explicitly calculated in Section \ref{sec:SMHeating} for hadrons, electrons, and photons.
...
The heating length of any such interaction is simply the result computed in Section~\ref{sec:SMHeating}. 
