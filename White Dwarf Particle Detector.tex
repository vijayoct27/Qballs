...
In particular, any DM candidate which couples to the SM will generically be able to release SM secondaries. 
...
The remaining nucleus is left in an excited state of energy $\sim E_\text{nuc}$ with negligible center-of-mass recoil \textcolor{blue}{cite}. 
Thus incident energies $E \gtrsim E_\text{nuc}$ are to leading order transfered entirely to the secondary hadrons, each of which will carry an $\OO(1)$ fraction of the incident energy \textcolor{blue}{cite}. 
If these secondaries also have energy above $E_\text{nuc}$, they are predominantly stopped by nonelastic nuclear collisions as well, and a roughly collinear hadron shower will result. 
...
The hadronic shower terminates in a cloud of final state nucleons and pions, each with energy $\sim E_\text{nuc}$. 
The energy loss of these particles is dominated by elastic nuclear scatters, which represents the end-stage of the ion heating process.
Thermalization will occur after the hadrons have scattered $\OO(1)$ of their energy, which requires $\sim 10$ scatters for nucleons and $\sim 100$ for pions. 
During this process the hadrons random walk over distance $\sim l_\text{h,el} \sqrt{N}$, with $ l_\text{h,el}$ the mean free path for elastic nuclear scattering and $N$ the number of scatters before for thermalization.
Thus nucleons thermalize over a distance $\sim l_\text{h,el}$ and pions over a slightly larger distance. 
As the shower will produce a near equal mix of nucleon and pion species, we conservatively take the products to thermalize over a distance $\approx 5~l_\text{h,el}$.

The hadronic shower is mostly collinear, so in the above language a single released hadron produces a temperature profile with $R_\text{h} \sim 10 ~l_\text{h,non}$ and $\Delta_\text{h} \sim 5 ~l_\text{h,el}$.
Specifically,
\begin{align}
    R_\text{h} &\sim 10^{-6} ~\text{cm}
        \l\frac{10^{32}~\text{cm}^3}{n_\text{ion}}\r \\
    \Delta_\text{h} &\sim 5\cdot10^{-8} ~\text{cm}
        \l\frac{10^{32}~\text{cm}^3}{n_\text{ion}}\r
\end{align}
...
Over the next two sections, we parameterize aspects of the DM-induced heating of WDs that are purely dependent on the nature of DM. 
...
We are able to constrain DM interaction parameters when the collision or decay releases total energy greater than $\eqref{eq:coldecay}$.
Here we focus on a schematic interaction where an annihilation/decay releases a number of SM particles $N_i$ of single species $i$ and energy $\epsilon$. 
As a result, the released particles will deposit its energy and thermalize ions within a heating length $L_0$, which was explicitly calculated in Section \ref{sec:SMHeating} for hadrons, electrons, and photons.
