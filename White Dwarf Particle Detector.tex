...
As discussed in Section \ref{sec:Review}, this is parameterized by the heating length $L_0$ of the deposition mechanism. 
In general, $L_0$ depends on the exact nature of the DM model.
However, a DM model that couples to the SM will generically allow heating of the star by releasing SM particles, as outlined in \ref{sec:DMexplode}. 
Calculating the heating lengths for such a process necessarily involves understanding the transfer of energy from individual SM particles to the stellar medium. 
In this section, we summarize the primitive heating properties of a single released SM particles of fixed energy $\epsilon$ and species $i$. 
...
For our purposed it will be sufficient to summarize this profile with two parameters: the displacement $R$ of the center of the profile from the origin of the SM particle, and the width $X$ of the temperature peak. 
...
For example, the decay of DM state into two SM particles will have $L_0 \sim X$.
However, for a transit process that releases many low-energy particles with $X \ll R$, the overall heating relies on the coherent addition of all of these individual deposits, and so $L_0 \sim R$. 

An essential ingredient for the analysis of this section is the stopping powers for SM particles in the WD medium. 
We will summarize here as needed the dominant stopping powers for each species along with the relevant physics of the dominant interactions.  
Computing these stopping powers and identifying the dominant contributions involves a number of subtleties due to the WD medium, which is explained in detail in Appendix \ref{sec:appendix}.
