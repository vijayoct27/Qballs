...
Evidently, either a few $N \sim 1$ ultra-high energy particles or a $N \gg 1$ large number of lower energy particles can be released, and either scenario will lead to different 
We summarize here the dominant source of energy loss for each species while a more detailed treatment of particle interactions in a WD can be found in Appendix \ref{sec:Appendix}. 
...
The primary quantity in calculating stopping power $(dE/dx)$, we define a length scale $\lambda$ which is the characteristic range over which that species would predominantly deposit its energy to either stellar constituents or to additional secondaries. 
To leading order $\lambda \sim (dE/dx)^{-1} E$ although more precisely, $\lambda$ is given by integrating the inverse stopping power over a relevant energy regime.  
...
Note that the heating properties of heavier SM states can be calculated in an analagous manner, although we do not do so in this work. 

\subsection{Heating Properties of Hadrons}

For incident hadrons of energy $\epsilon > 10 ~\text{MeV}$, the energy loss is dominated by inelastic nuclear collisions in which an incoming hadron strongly interacts with a target nuclear constituent to produce several secondary hadrons.
...
First we consider the energy loss of hadrons with energy less than the nuclear binding energy $\epsilon \lesssim 10 ~\text{MeV}$. 
At these energies, protons and neutrons are stopped predominantly through elastic nuclear scatters, characterized by a typical elastic cross section of order $\sigma_\text{el} \sim 100 ~\text{mb}$. 
...
Such high densities will result in the efficient stopping of incident particles while also giving rise to qualitatively different effects than seen in terrestrial detectors.  
...
\end{equation}
For a heavy particle incident on an electron target this reduces to $W_\text{kin} \approx 2 m_e \beta^2 \gamma^2$.
...
In this case, the Coulomb stopping power is parametrically
\begin{align}
\label{eq:SP}
...
Namely, energy transfers below this threshold will occur via the creation of phonon excitations and will be suppressed from the free-particle results used above. 
% Probably something like using a phonon cross section for W < \omega, and a free-particle Coulomb cross-section otherwise. 
% If W_TF < \omega, do we stll need the W_TF cutt-off for phonon scattering?
...
Although one must consider the relativistic velocities of the degenerate electrons, to leading order we can simply average over this motion and treat the targets as stationary. 
Note that electron degeneracy now plays a significant role as an incident particle transferring energy $W$ can only scatter electrons within $W$ of the Fermi surface.
...
For an incident electron, we additionally demand that the energy remains above the Fermi surface after scatting, that is $W < E - E_F$. 
Furthermore, the cross-section \eqref{eq:mott} is also no longer fully valid in the case of electron-electron scattering as we must account for a significant target recoil as well as identical particle effects (i.e., the M{\"o}ller cross-section).
However, after integrating to find the stopping power, we find that such a proper treatment only introduces $\OO(1)$ corrections. 
In summary, the stopping power of any charged particle scattering off electrons is roughly constant at sufficiently high energies
\begin{equation}
-\l \frac{dE}{dx} \r \sim \frac{2 \pi n_e \alpha^2}{m_e}. 
\end{equation}
However, the low-energy behavior is more complicated and requires the full calculation taking into account Pauli-blocking and charge screening effects. 
...
Bremsstrahlung and pair production should also be considered as stopping mechanisms for high-energy electrons and photons. 
...
The nuclear fragment is typically left in an unstable state with negligible center-of-mass recoil, and relaxation occurs via the slow emission of low-energy $\sim \MeV$ hadrons and photons. 
