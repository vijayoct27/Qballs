...
Incident hadrons with kinetic energies greater than the nuclear binding scale $E_\text{nuc} \sim 10~\MeV$ will undergo violent inelastic collisions with carbon ions resulting in an $\OO(1)$ number of secondary hadrons.
These secondaries will consist of roughly equal fractions of pions, protons, and neutrons, each carrying an $\OO(1)$ of the incident energy.
\textcolor{blue}{cite something}
These secondaries will undergo similar collisions, resulting in a roughly collinear shower terminating when the shower constituents reach an energy $\sim E_\text{nuc}$.
This occurs over a disance 
\begin{align}
\label{eq:hadlength}
  X_\text{had} \sim l_\text{inel} \log\l\frac{\epsilon}{E_\text{nuc}}\r
  \approx 10^{-6} ~\text{cm} \l\frac{10^{32}~\text{cm}^{-3}}{n_\text{ion}}\r
\end{align}
where $l_\text{inel}$ is the mean free path for nuclear scatters, set by the cross-section $\sigma_\text{inel} \approx 100 ~\text{mb}$.
\textcolor{blue}{cite something}
...
First, however, we review the relevant WD observables and consider the different ways these can be used to constrain DM candidates capable of triggering SN.
