...
This unknown DM physics determines the distribution in momentum, species, and space of the SM particles produced in the star, which must be known in order to determine if a given DM-WD encounter results in a SN, and further to calculate the rate of such SN. 
...
We describe these interactions below, compute their DM-induced SN rates, and give schematic constraints.

A DM candidate may be generically expected to interact with the WD medium through the three processed depicted in Figure \ref{fig:feynman}: DM-DM collisions, DM decays, and DM-SM scattering. 
Note that for ultra-heavy DM these processes may not be elementary interactions, but could rather be more complicated events involving many final states, analogous to the interactions of high-energy hadrons or heavy nuclei. 
We classify DM candidates into three types according to the interaction that provides the dominant source of heating.
We also make some simplifying assumptions about the spatial extent of these interactions. 
We consider only decays and collisions that are point-like, with all SM products are produced in a localized region (smaller than the trigger size), and we take the transit processes to involve only soft DM-SM scatters, resulting in the release of SM energy continuously along the DM trajectory. 
...
For some candidates this accumulated DM may have an enhanced likelihood to cause ignition, so models that undergo capture may be more strongly constrained.   
