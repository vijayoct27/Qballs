...
However, high-energy radiative processes are significantly suppressed in the WD medium by the Landau-Pomeranchuk-Migdal (LPM) effect. Highly-energetic electromagnetic interactions involve small longitudinal momentum transfers 

 reactions occur gradually, spread over long distances. During this time, even relatively weak factors can accumulate enough to disrupt the interaction.The longitudinal momentum transfer between a bremsstrahlung electron and the target nucleus is $q_{||} = km_e^2/2E(E-k)$. For pair production, $q_{||} = km_e^2/2E(k-E)$. This interaction is delocalized over a formation length $l_f \sim 1/q_{||}$ and so the amplitudes for all interactions must be added within this distance - if this scatters the electron or photon significantly, then bremsstrahlung/pair production will be reduced. 

For bremsstrahlung and pair production, 

\begin{equation}
\label{eq:LPM}
E_\text{LPM} = \frac{m^2 X_0 \alpha}{4 \pi}. 
\end{equation}
