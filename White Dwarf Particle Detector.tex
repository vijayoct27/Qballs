...
In Section~\ref{sec:DMexplode} we derive conditions on the properties of DM necessary to ignite white dwarfs, focusing on heating through annihilation, decay, and transit processes. 
We constrain DM models parameterized in this way in Section~\ref{sec:Constraints}, and examine the specific case of Q-balls in Section~\ref{sec:QBalls}.
...
In this section we parameterize the aspects of DM-induced heating of WDs through non-gravitational interactions that are purely dependent on the nature of the DM. 
In particular, we derive explosion conditions on potential DM-WD encounters that determine whether or not they are capable of igniting the star. 
This is done for the three simple and illustrative examples depicted in Figure \ref{fig:feynman}: DM-DM collisions, DM decays, and DM transits. 
...
With this parameterization, the explosion condition depends only on the DM mass the heating length for the process:
\begin{equation}
\label{eq:coldecay}
    m_\text{DM} \gtrsim \mathcal{E}_\text{boom} \cdot \frac{1}{\eta}
      \text{max}\left\{1, \frac{L_0}{\lambda_T}\right\}^3
\end{equation}
Considering the threshold \eqref{eq:Eboom}, such events require DM masses larger than $\sim 10^{15} ~\GeV$ in order to release sufficient energy to trigger a SN.
...
We parametrize the DM kinetic energy loss per distance travelled in a WD by a DM stopping power $(dE/dx)_\text{SP}$ and restrict our attention to ``bullet-like" transits. 
...
We now turn towards constraining models of ultra-heavy DM which can ignite WDs via one of the processes described in Section~\ref{sec:DMexplode}. 
