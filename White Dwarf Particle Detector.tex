...
\label{sec:DMexplode}
In this section, we consider the conditions necessary for DM interactions to explode a WD. In particular, we consider generic (ultra-heavy) DM interactions in the WD which release $N_i$ SM particles of species $i$ each with kinetic energy $\epsilon$.
...
\end{figure}
Setting the total energy released during a transit of length $\text{max}\{\lambda_T, L\}$ to be greater than the explosive energy \eqref{eq:boom}, we find a lower bound on the interaction cross section sufficient to trigger runaway fusion:
\begin{equation}
...
Considering these conditions and the data available to us from a white dwarf observation, the independent parameters we are free to vary and constrain are the species $i$ released during a DM-SM collision, the weighted cross-section $N_i \epsilon\, \sigma_{\epsilon,i}$, the local mass density of the DM candidate $\rho_\text{DM}$, and the dark matter mass $m_\text{DM}$. The relevant heating length $L$ is then determined by $i$, $\epsilon$, and white dwarf parameters, and parameter constraints follow.

\begin{figure}
  \includegraphics[scale=0.65]{dummy_constraint_plot.png}
  \caption{A placeholder plot illustrating how we may want to do the constraint plots for various white dwarf observations and released particle species.}
...
The bremsstrahlung cross section for an electron of energy $E$ to radiate a photon of energy $k$ in a medium of nuclear targets with number density $n$ is given by the semi-classical Bethe-Heitler formula
\begin{equation}
\label{eq:BH}
...
\begin{equation}
X_0^{-1} = 4 n Z^2 \frac{\alpha^3}{m_e^2} \log{\Lambda}, ~~~ \log{\Lambda} \sim \int \limits_{b_{min}}^{b_{max}} \frac{1}{b}.
\end{equation}
where $\log{\Lambda}$ is a logarithmic form factor containing the maximum and minimum effective impact parameters allowed in the scatter. $b_{min}$ is set by a quantum-mechanical bound such that the radiated photon frequency is not larger than the initial electron energy. For a bare nucleus, this is simply the electron Compton wavelength
\begin{equation}
b_{min} = \lambda_e = \frac{1}{m_e}.
\end{equation}
It is important to note that collisions at impact parameters \emph{less} than $b_{min}$ will still radiate, but with exponentially suppressed intensity. Classically, this is manifested by the decoherence of radiation at large scattering angles. For relativistic electrons, screening is important over almost the entire range of $k$ and shields the radiating electron from the nucleus at large distances. Therefore, the maximum impact parameter is set by the distance at which the nuclear target is screened. For an atomic target, this is of order the Bohr radius. In the case of a WD, this is set by the Thomas-Fermi length \eqref{eq:TF}:
\begin{equation}
b_{max} = \lambda_\text{TF} = \l \frac{6 \pi Z \alpha n_e}{E_F}\r^{-1/2}.
\end{equation}
Immediately it is evident that there exists a critical number density at which the Thomas-Fermi length is of order the Compton wavelength. Roughly this occurs at an electron number density $n_{crit} \sim 10^{32} ~\text{cm}^{-3}$. For larger densities, the bremsstrahlung amplitude will be highly suppressed. For our purposes, we simply take $\log{\Lambda} \sim \OO(1)$ whenever $b_{min} \lesssim b_{max}$.
...
\end{equation}
Note that unlike bremsstrahlung, pair production has a threshold energy so that \eqref{eq:PP} is only valid for high-energy radiation $k \gtrsim m_e$. Integrating \eqref{eq:BH}, the energy loss of a high-energy electron is simply
\begin{equation}
\frac{dE}{dx} = \frac{E}{X_0},
\end{equation}
and the full pair production cross section is $\sigma_{pp} = \frac{7}{9} \frac{1}{n X_0}$. $X_0$ is the typical length scale of a high-energy electromagnetic shower.

However, bremsstrahlung and pair production will be suppressed by the ``Landau-Pomeranchuk-Migdal" (LPM) effect (see \cite{LPM} for an extensive review). High-energy radiative processes involve very small longitudinal momentum transfers to a nuclear target ($\propto k/E^2$ in the case of bremsstrahlung). Quantum mechanically, this interaction is delocalized across a formation length over which amplitudes from different scattering centers will interfere. This interference is usually destructive and must be taken into account in the case of ultra-high energies or high-density mediums.

Calculations of the LPM effect can be done semi-classically based on average multiple scattering. It is found that bremsstrahlung is suppressed for $k > (E-k)^2/E_\text{LPM}$ and pair production for $E(k-E) > k E_\text{LPM}$, where
\begin{equation}
\label{eq:LPM}
E_\text{LPM} = \frac{m_e^2 X_0 \alpha}{4 \pi}.
\end{equation}
For the WD densities under consideration, $E_\text{LPM} \sim 10-10^{3} ~\text{MeV}$. Note that $E_\text{LPM}$ generally scales as the fourth power of the mass of the incident particle so for muons, $E_\text{LPM} \sim 10^{10}-10^{12} ~\text{MeV}$.
We find that the bremsstrahlung stopping power with LPM suppression is parametrically
\begin{equation}
\l\frac{dE}{dx}\r_\text{LPM} \sim \l\frac{E_\text{LPM}}{E} \r^{1/2} \frac{E}{X_0}.
\end{equation}
Qualitatively, the LPM suppression diminishes the energy loss due to soft radiation so that the stopping power is dominated by \emph{hard} bremsstrahlung.
Similarly, the pair production cross section reduces to
\begin{equation}
\sigma_\text{LPM} \sim \l\frac{E_\text{LPM}}{k} \r^{1/2} \frac{1}{n X_0}.
\end{equation}

In addition to multiple scattering, additional interactions within a formation length will suppress bremsstrahlung when $k \ll E$. The emitted photon can coherently scatter off ions in the media, acquiring an effective mass of order the ion plasma frequency $\Omega_p \sim \l \frac{n_i Z^2 e^2}{m_i} \r^{1/2}$.  \textcolor{red}{fastest plasma frequency will dominate - electrons have the same form if degenerate?}. Semi-classically, this results in a suppression of order $(k m_e/\Omega_p E)^2$ when the radiated photon energy $k < \Omega_p E/m_e$. This is typically known as the ``dielectric effect", and is responsible for regulating the log divergence of the bremsstrahlung cross section for soft radiation. In the WD, the typical ion frequency is in the range $\Omega_p \sim 1 -10 ~\text{keV}$. However, when the LPM suppression is included, the radiative stopping power is less sensitive to the dielectric effect. For our purposes, this can be ignored.
...
High-energy $k \gtrsim 10 ~\text{MeV}$ photons can also interact to nuclei and induce a hadronic shower. There is considerable uncertainty in the evaluation of high-energy photonuclear cross sections, and a precise formulation is beyond the scope of this work. As a conservative estimate, we suppose a constant value of order
\begin{equation}
