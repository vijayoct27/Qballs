...
In this paper we extend the capability of white dwarf detectors to dark matter candidates with non-gravitational couplings which release energy in the form of standard model particles.
This is used to constrain dark matter collisions, decays, and transits that are capable of igniting white dwarfs. 
...
The necessary trigger for runaway fusion was initially computed in \cite{Woosley} and recently implemented in \cite{Graham:2015apa} to constrain primordial black holes. 
...
In this work, we extend their analysis to generic models of DM with non-gravitational couplings which release energy in the form of high-energy standard model (SM) particles.
DM candidates with interactions of this sort include baryonic Q-balls found in supersymmetric extensions of the SM and models of dark nuclei with higher-dimension couplings to the SM.
As such, the WD may be thought of as a particle detector with hadronic and electromagnetic ``calorimeter'' components.
This is used to constrain DM models with DM-DM collisions, DM decays, and DM-SM scattering processes that are capable of igniting white dwarfs. 
...
The former can be viewed as a ``flux is too low" fundamental limit for the WD as a DM detector, while the latter can be viewed as a ``detection threshold" for signal over background.  
...
These result in a lighter, excited nucleus and a few outgoing hadrons which roughly carry away the incident particle's kinetic energy in equal fractions. \textcolor{blue}{citation}
...
For DM models with a DM-SM scattering interaction, there may be a continuous production of SM particles as the DM traverses a WD.
...
We now turn towards constraining generic models of ultra-heavy DM which can ignite WDs via one of the processes described in Section~\ref{sec:DMexplode}. 
...
The NuStar collaboration has also recently uncovered evidence for the likely existence of $\sim 1.25 ~M_{\odot}$ WDs in the galactic center as well \textcolor{blue}{get citations from SR}.
