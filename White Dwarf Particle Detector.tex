...
Note that we will always have $R_i \gtrsim X_i$, and indeed it turns out that for most species $R_i \sim X_i$, with neutrinos begin the one unsurprising exception.
Either of the quantities $R$ and $X$ may set the scale of the heating length of a given encounter depending on their relative sizes and on the number of particles involved, so it is important to understand them both. 
If only a few SM secondaries are produced, e.g.\ in a DM decay, then $L_0 \sim X_i$ as we focus only on regions heated by the individual decay products.
