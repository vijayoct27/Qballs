...
A DM transit releases energy continually along the DM particle's trajectory, resulting in a cylindrical temperature profile with some transverse length scale $L$.  The energy available to deposit will come from the kinetic energy of the DM, and so by the ignition thresholds in Figure \ref{fig:Eboom} we ought to consider only very heavy DM particle Ignition will be determined by both $L$ and the amount of energy deposited per length along the DM trajectory, so we parameterize this interaction in terms of a \emph{stopping power} $\chi$:
\[
    \chi = \frac{1}{n} \frac{\partial{E}}{\partial{x}}
\]
where $n$ is the number density of the stellar medium and $\partial{E}/\partial{x}$ is the energy lost by the DM per distance traveled.  Nominally, the energy loss $\partial{E}/\partial{x}$ depends on $n$, the energy of the DM particle,

We expect the energy loss $\partial{E}/\partial{x}$ to scale linearly with the $n$, which will be justified below, and so $\chi$ is a function only of the DM energy and the details of its interactions with the SM.  Additionally, the initial since each DM particle will transit the star with a speed that is $\OO(1)$ the star with particular interaction with the   Normalized in this manner, $\chi$ is independent of $n$ and depends only the parameters of the particular DM model under consideration.  The stopping power has the interpretation of an interaction cross-section weighted by the energy transfered in that interaction.  For example, if the DM has a cross-section $\sigma$ to interact with a carbon ion to produce $N$ SM particles of energy $\epsilon$ each, then $\chi = N \sigma \epsilon$.  In general $\chi$ is given by an integral over all possible outgoing SM states.  We will use the terminology of \emph{stopping power} as this $\chi$ is the same quantity that is studied to determine the stopping of energetic particle in matter.  However, in all cases where a DM transit is capable of ignition it will turn out that only a small fraction of the DM's kinetic energy is transfered to the star before ignition, so we ignore any change in the DM velocity. 
...
High-energy $k \gtrsim 10 ~\text{MeV}$ photons can also interact with nuclei and induce a hadronic shower. There is considerable uncertainty in the evaluation of high-energy photonuclear cross sections, and a precise formulation is beyond the scope of this work. As a conservative estimate, we take a constant photonuclear mean free path and cross section: 
\begin{equation}
l_\gamma \sim \frac{1}{n \sigma_{\gamma A}}, ~~~~ \sigma_{\gamma A} \sim 1 ~\text{mb}.
\end{equation}
