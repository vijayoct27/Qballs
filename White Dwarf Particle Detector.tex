...
Here we simply state the results: if ions are heated to temperatures $\sim 1 - 10 ~\MeV$, then electrons and photons are also both efficiently heated within the trigger size. 
...
Note that \eqref{eq:heatcapacity} uses the heat capacity of a degenerate gas of electrons as the Fermi energy $E_F \gtrapprox \MeV$ for the densities we consider.
...
As a result, understanding the \emph{heating lengths} $L_0$ for different types of heating events in a WD is critical to determining whether they are capable of destroying the star.
\begin{figure}
