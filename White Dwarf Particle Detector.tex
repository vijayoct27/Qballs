...
Within this schematic for the DM-WD interaction, one can now constrain the parameter $\sigma_{i,\epsilon} \epsilon$ as a function of DM mass $m_\text{DM}$. Note that this limit can be placed over a range of produced energies $\epsilon \sim 10^{6} - 10^{18} ~\text{MeV}$ using the values of $L_i (\epsilon)$ calculated in Section \ref{sec:HeatingLength}. As an example, this is done for released photons and electrons in Figures \ref{fig:photon} and \ref{fig:electron} for a WD of mass $\sim 1.25 M_\odot$.
