...
Of course, if ions were the only WD constituents to establish a temperature profile at this scale then $\Ez \sim n_\text{ion} T_0 L_0^3$, where $n_\text{ion}$ is the number density of ions.
...
\begin{equation}
\label{eq:boom}
  \Ez \gtrsim \sum n_\text{ion} T_f \text{max}\{L_0, \lambda_T\}^3.
\end{equation}
...
\begin{equation}
\label{eq:Eboom}
\Eboom \sim n_\text{ion} T_f \lambda_T^3 \sim 10^{15} - 10^{20} ~\GeV,
\end{equation}
...
\label{eq:transitexplosion}
  \left( \frac{d E}{d x} \right)_\text{LET} \gtrsim \frac{\Eboom}{\text{max} \{\lambda_T, L_0\}}.
\end{equation}
...
\end{equation}
Therefore $\sigma_Q \approx 10^{-12} ~\text{cm}^2$ is sufficient to blow up a  $\sim 1.25 ~M_{\odot}$ WD, so if the Q-ball cross-section is related to its mass and baryonic charge as in \eqref{eq:Qballprop}, we find that $Q \gtrsim 10^{42} ~(m_S/\text{TeV})^4$ can be constrained.
