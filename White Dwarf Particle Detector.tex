...
Therefore, a heated region with temperature $T$ greater than $T_f$ and size $R$ greater than $\lambda_T$ in a sub-Chandrasekhar WD will eventually launch a runaway fusion chain-reaction and result in a type Ia supernova (SN):
\begin{equation}
\label{eq:runaway}
...
In the other extreme, suppose that a process produces electrons just above the Fermi energy.
...
We focus first on a incident incident photon losing energy to the WD medium.
Since the cross-section for this process scales as $\sim Z^2 \alpha^2/m^2$ for a target mass $m$ and charge $Z$, we ignore photon-ion scatters in favor of photon-electron scatters. 

Consider an incident photon of energy $k$ scattering off a electron of energy $E \sim E_F$.
In the rest frame of the electron, this cross-section is given by the the Klein-Nishina formula,
\begin{equation}
\label{KN}
...
\end{equation}
where $k^\prime$ is the outgoing photon energy, related to the scattering angle by Compton's formula,
\begin{equation}
{k^{\prime }={\frac {k}{1+{\frac {k}{m_e}}(1-\cos \theta )}}}.
\end{equation}
We need only consider the case $k > m$. 
In this limit the cross-section is suppressed by the incoming energy $k$:
\begin{equation}
\label{eq:KN-highenergy}
  \sigma \sim \frac{\alpha^2}{m_e k}.
\end{equation} 
The outgoing photons will scatter predominately in a near-forward direction, $\cos \theta \approx m/k$, with an outgoing energy $k^\prime \sim m$.
...
A more careful analysis computes the stopping power via 
\begin{equation}
\label{eq:comptonSP}
  \frac{dk}{dx} =  \int d (\cos \theta) n_e \frac{d\sigma_\text{KN}}{d (\cos \theta)} \l k - k' \r, 
\end{equation}
taking into account the Lorentz boost to the electron's rest frame.
This does not alter the result beyond $\OO(1)$ factors, since the target electrons are only slightly relativistic, $\gamma_e \lesssim 10$.
Further, Paul-blocking of the target electrons should be taken into account via \eqref{eq:pauliblocking}, although we find that degeneracy only introduces a significant suppression when $k\lesssim 10 ~\text{MeV}$.
This is also expected, as the interaction is dominated by hard, near-forward scatters.

We now consider briefly to incident electrons which may cool by (inverse) Compton scatters with the thermal bath of photons in the WD.  
The number density of these photons is set by the thermal temperature of the star, $n \sim T^3$, which for temperatures of $\keV$ gives $n \sim 10^{23} \cm^{-3}$.
...
An estimate in the manner of the above calculation (including the now-significant boost factors) gives the inverse Compton stopping power in terms of the photon temperature $T \sim \keV$ and incident electron energy $E$:  
\begin{equation}
\label{eq:invcomptonSP}
  ...
  \end{cases}.
\end{equation}
Where the change in scaling with $E$ marks a transition from Thompson-like scattering in the electron rest frame to suppressed high-energy scattering (the regime studied above).
