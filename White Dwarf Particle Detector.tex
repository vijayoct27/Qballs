...
A DM transit releases energy continually along the DM particle's trajectory, and so it is best described by a \emph{stopping power} $\chi$ defined as:
\[
    \chi = \frac{1}{n} \frac{\partial{E}}{\partial{x}}
\]
where $n$ is the number density of the stellar medium and $\partial{E}/\partial{x}$ is the energy lost by the DM per length traveled.  Normalized in this manner, $\chi$ is independent of $n$ and depends only the parameters of the particular DM model under consideration.  The stopping power has the interpretation of an interaction cross-section weighted by the energy transfered in that interaction.  For example, if the DM has a cross-section $\sigma$ to interact with a carbon ion to produce $N$ SM particles of energy $\epsilon$ each, then $\chi = N \sigma \epsilon$.  In general $\chi$ is given by an integral over all possible outgoing SM states.  We will use the terminology of \emph{stopping power} as this $\chi$ is the same quantity that is studied to determine the stopping of energetic particle in matter.  However, in all cases where a DM transit is capable of ignition it will turn out that only a small fraction of the DM's kinetic energy is transfered to the star before ignition, so we ignore any change in the DM velocity. 
