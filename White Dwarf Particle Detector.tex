...
For instance, DM masses above $\sim 10^{22} ~\GeV$ will register fewer than a single event per year in a typical detector of size $\sim (100 ~\text{m})^2$.
...
These will deposit their energy and thermalize ions within a distance explicitly calculated in Section \ref{sec:SMHeating}. 
With the above schematic for DM-DM collisions and DM decays, we can place bounds on the cross-section $\sigma_\text{DM-DM}$ or lifetime $\tau_\text{DM}$ as a function of $m_\text{DM}$.
...
As we have not seen any cosmic rays with energy greater than $\sim 10^{12} ~\GeV$ \cite{ThePierreAuger:2015rha, AbuZayyad:2012ru}, this places a similar constraint on the DM interaction parameters $\sigma_\text{DM-DM}$ and $\tau_\text{DM}$. 
For instance, one can estimate the expected cosmic ray flux due to a DM decay event in the galactic halo as follows. 
