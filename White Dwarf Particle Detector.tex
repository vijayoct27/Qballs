...
Any energy deposit will eventually heat ions within some localized region---parameterize this region by its peak temperature $T_0$ and linear size $L_0$.
...
Of course, the relation between $\Ez$ and $T_0$ depends on the rate at which WD constituents (ions, electrons, and photons) interact with each other. 
We explicitly calculate the length scale over which a hot bath of ions thermalizes electrons and the length scale over which a hot bath of electrons thermalizes photons in Section \ref{sec:SMHeating}. 
Here we simply state the results: if ions are heated to a temperature $\sim 1 - 10 ~\MeV$, then electrons and photons are also both heated within the trigger size. 
Therefore, any event which results in runaway necessarily has ions, electrons, and photons in thermal equilibrium once a region of size $\lambda_T$ or greater is at the critical temperature $T_f$. 
The excess energy in a volume $V$ required to heat all species to $T_f$ is given by a sum of their heat capacities
\begin{equation}
...
Note that \eqref{eq:heatcapacity} uses the heat capacity of a degenerate gas of electrons as the Fermi energy $E_F \gtrsim \MeV$ for the densities we consider.
We find the minimum energy deposit necessary to trigger runaway fusion is
\begin{align}
\label{eq:Eboom}
...
Thus for a general heating event characterized by $L_0$ and $\Ez$, there is a \emph{boom condition}:
\begin{align}
...
Understanding the \emph{heating lengths} $L_0$ for different types of heating events in a WD is therefore critical to determining whether or not these are capable of destroying the star.
\begin{figure}
\includegraphics[scale=.45]{Eboom.pdf}
\caption{The minimum energy deposit $\Eboom$ \eqref{eq:Eboom} necessary to trigger runaway fusion, based on numerical results for the trigger size $\lambda_T$ \cite{Woosley} and WD mass-density relation \cite{cococubed}}.
\label{fig:Eboom}
\end{figure}
