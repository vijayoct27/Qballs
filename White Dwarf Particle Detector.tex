...
This is primarily aimed towards calculating the stopping of electrons, photons, and light hadrons (protons, neutrons, pions) within the stellar medium at energies greater than $\sim \text{MeV}$. 
...
\end{equation}
<<<<<<< HEAD
where $n_e \sim 10^{30} - 10^{33} ~\text{cm}^{-3}$ is the number density of electrons (we take $\hbar = c = 1$ throughout). 
...
\end{equation}
<<<<<<< HEAD
where $n_\text{ion} \sim 10^{29} - 10^{32} ~\text{cm}^{-3}$ is the number density of nuclei \cite{Teukolsky}. 
=======
...
\end{equation}
<<<<<<< HEAD
For light targets such as electrons $M = m_e \ll m$, this reduces to $W_\text{kin} \approx 2 m_e \beta^2 \gamma^2$, while for electron-electron scattering this is the full energy of the incident electron with a factor of 1/2 to account for the identical particles.
...
The minimum energy transfer $W_\text{min}$ should be set by the impact parameter at which charge screening becomes relevant. 
In a WD, the screening length $\kappa_{\text{sc}}^{-1}$ due to the degenerate electron gas can be calculated in the Thomas-Fermi approximation
\begin{equation}
\label{eq:TF}
...
Here we discuss particle interactions in the WD due the strong force. 
...
In the case of neutrons, $\sigma_\text{el}$ is dominated by nuclear resonances in the regime $1 - 10 ~\text{MeV}$ is still roughly $\sigma_\text{el} \approx 3$
