...
The relation between $\Ez$ and $T_0$ depends on the rate at which WD constituents (ions, electrons, and photons) thermalize with each other. 
This is non-trivial, and we explicitly calculate these interactions and their associated thermalization length scales in Section \ref{sec:SMHeating}. 
Importantly, we find that if a heating event raises the temperature of ions to $T_0 \gtrsim T_f$ in some region $L_0$, electrons and photons will also be heated rapidly within a size $\lambda_T$.
Thus any heating event capable of runaway necessarily has ions, electrons, and photons in thermal equilibrium. 
...
Note that this uses the heat capacity of a degenerate gas of electrons as the Fermi energy $E_F$ is either greater than or of order $T_f$ for the densities we consider.
Therefore the minimum energy deposit needed for ignition is
\begin{align}
\label{eq:Eboom}
...
In terms of $\Eboom$, the boom condition~\eqref{eq:Tboom} can be restated as
\begin{align}
...
Understanding the heating lengths $L_0$ resulting from the production of different SM particles is therefore critical to determining whether or not these processes are capable of destroying a WD.
\begin{figure}
