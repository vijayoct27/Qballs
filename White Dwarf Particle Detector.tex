...
\begin{align}
  \label{eq:omega_soft}
  \omega \approx \frac{e^2 Z^2}{2 m_c \beta^2 b^2}
\end{align}
...
\begin{align}
    ...
    \end{dcases}
\end{align}
where $b_0^2 = e^4 Z^2 / 2 m_c \beta^2 \omega_\text{kin}$ is a characteristic transition scale between hard and soft scatters. 
...
\begin{align}
  \frac{dE}{dx} &\approx n_\text{ion} 
    ...
    &\approx 2 \pi n_\text{ion} \left( \omega_\text{kin} \int_0^{b_0} \, db 
    + \frac{e^4 Z^2}{2 m_c \beta^2}  
    \int_{b_0}^{\lambda_{TF}} \,\frac{db}{b} \right)  \nonumber \\
    &= \begin{dcases}
      n_\text{ion} \omega_\text{kin} 2 \pi \lambda_{TF}^2 
        & \lambda_{TF} < b_0 \\
      n_\text{ion} \omega_\text{kin} 2 \pi b_0^2 \left[ 1 
        + \log\left(\frac{\lambda_{TF}}{b_0}\right)
        \right] & \lambda_{TF} > b_0 \nonumber 
  \end{dcases}
\end{align}
\textcolor{blue}{plug shit in}
