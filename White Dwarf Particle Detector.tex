...
Under the above assumptions, one can now constrain the (model-dependent) parameter $N_i \sigma_{i,\epsilon} \epsilon$ as a function of DM mass $m_\text{DM}$. Note that this limit can be placed over a range of produced energies $\epsilon \sim 10^{6} - 10^{18} ~\text{MeV}$ using the calculated values of $L_\text{electron}$ and $L_\text{photon}$ from Section \ref{sec:HeatingLength}. As an example, this is done for $i=\text{photon}$ and $i = \text{electron}$ in Figures \ref{fig:photon} and \ref{fig:electron} for a WD of mass $\sim 1.25 M_\odot$.
