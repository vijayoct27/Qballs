...
The cross section for elastic scatters has considerable dependence on the incident particle species. For neutrons and protons, the nuclear elastic cross section is of order $\sigma_\text{el} \sim 100 ~\text{mb}$ for energies greater than $\sim \text{MeV}$. However at lower energies, the elastic cross section effectively vanishes for protons but rises to $\OO(\text{b})$ for neutrons. This can be understood physically as charged hadrons may only elastically scatter off nuclei if the incident energy is sufficient to overcome the mutual Coulomb barrier. Note that the neutron capture cross section in carbon and oxygen is negligible compared to the elastic cross section. The stopping power due to nuclear elastic collisions can be expressed as
\begin{equation}
\label{eq:elastic}
\l\frac{dE}{dx}\r_\text{el} \sim E  (1-\zeta) ~n \sigma_\text{el},
\end{equation}
valid for non-relativistic nucleons traversing in the target in the form of a random-walk. However, since the ions in a WD form a Coulomb lattice with binding energy \eqref{eq:lattice}, elastic collisions cannot transfer energy less than $\sim E_B$ without exciting phonon modes. This limits the applicability of \eqref{eq:elastic} to energies below an $\sim \text{MeV}$ for both neutrons and protons. 
...
There is considerable uncertainty in the evaluation of nuclear cross sections for high-energy particles, and a precise formulation is beyond the scope of this work. As a conservative estimate, we take the nonelastic nuclear cross sections for nucleons and photons to be constant (in the relevant energy regime):
\begin{equation}
\sigma_{n A} \sim 100 ~\text{mb}, ~~~\sigma_{\gamma A} \sim 1 ~\text{mb}.
\end{equation} 
To determine the typical hadronic shower length $X_{\text{had}}$ initiated by species $i$, we assume the primary particle of energy $\epsilon$ interacts with nuclei to produces $N$ secondary particles each of energy $\epsilon/N$ (we ignore the effects of the evaporative stage in each reaction as this will only introduce minor corrections at sufficiently large $\epsilon$). The shower will end once final-state hadrons reach a critical energy $E_\text{crit}$ - this is either set by the scale at which an additional mechanism dominates the stopping power or the minimum energy required to induce further showers $E_\text{nuc}$. Over this range of energies, the stopping power for nucleons is roughly
\begin{equation}
\l \frac{dE}{dx}\r_\text{nuc} \sim E n \sigma_{n A},
\end{equation}
yielding a hadronic shower length of order
\begin{equation}
X_{\text{had}} \sim l_\text{n} \l \frac{\log{(\epsilon/E_c)}}{\log{N}}\r \approx 10 ~l_\text{n}.
\end{equation}
Note that the shower length has a mild, logarithmic sensitivity to the initial energy $\epsilon \gg ~\text{10 ~\text{MeV}}$ and number of secondaries and is therefore primarily set by the nuclear mean free paths. In the case of an incident hadron (proton, neutron, pion), the hadronic shower length is approximately set by a single length scale $X_{\text{had}} \approx 10 ~l_\text{had}$.

Charged leptons can also lose energy via electronuclear interactions in which an incident electron or muon radiates a virtual photon that then interacts hadronically with a nearby nucleus. This radiative stopping power has been computed in \cite{Gerhardt:2010bj} in terms of the photonuclear mean free path $l_\gamma = \frac{1}{n \sigma_{\gamma A}}$:
\begin{equation}
