...
Note that the size and thermal properties of any heated region will evolve in time, so we take $L_0$ and $\Ez$ to refer to this peak as it initially forms, i.e.\ immediately after a temperature profile for ions has been established.
Thus for a given heating event, the heating length $L_0$ is a measure of the efficiency with which the energy deposit is transferred to ions in the stellar medium.
...
This ion would thermalize with neighboring ions, resulting in a heating length of order the ion mean free path.
...
The precise outcome is governed by two parameters: the fusion temperature $T_f$ and the trigger size $\lambda_T$.
The fusion temperature $T_f$ is the threshold for the onset of fusion and is set by the energy required for ions to overcome their mutual Coulomb barrier.
...
Sustained heating through nuclear fusion can only be maintained if the timescale for cooling is consistently larger than the timescale for heating. 
...
In summary, a local temperature peak $T_f$ in a WD will either always be dominated by diffusive cooling if $L_0 < \lambda_T$ or runaway fusion if $L_0 > \lambda_T$.  
...
We can assess this outcome by assuming the region with $T_0 > T_f$ begins to diffuse and cool until its temperature is of order $T_f$.
...
As a result, understanding the heating length $L_0$ of a processes is critical to determining whether or not it is capable of destroying a WD. 
...
\section{Heating Properties of Standard Model Particles}
\label{sec:SMHeating}

As discussed in Section~\ref{sec:Review}, a heating event must efficiently transfer energy to ions in order to trigger runaway fusion, parameterized by the heating length $L_0$ of the event. 
We will ultimately parametrize 
