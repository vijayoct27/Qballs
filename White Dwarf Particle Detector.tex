...
This large density provides efficent stopping of incident particles, while also giving rise to qualitatively different stopping behavior than seen in terresrial detectors. 
Famously, the star is supported against collapse by the electron degeneracy pressure with a characteristic Fermi energy
\begin{equation}
...
A particle of mass $m$, charge $e$, and velocity $v$ incident on a stationary target of mass $M$ and charge $Ze$ will transfer an energy $W$ as given by the Mott cross-section 
\begin{equation}
\label{eq:QED}
...
A particle of mass $m$, charge $e$, and velocity $v$ incident on a stationary target of mass $M$, charge $Ze$ at impact parameter $b$ will transfer energy
\begin{equation}
\label{eq:impact}
...
\begin{equation}
\label{eq:rutherford}
\frac{d \sigma_R}{dW} = \frac{2 \pi  \alpha^2 Z^2}{M v^2} \frac{1}{W^2},
\end{equation}
...
\begin{equation}
\label{eq:QED}
\frac{d \sigma_R}{dW} = \frac{2 \pi  \alpha^2 Z^2}{m_e v^2} \frac{1}{W^2} \l1-v^2 \frac{W}{W_\text{kin}} \r,
\end{equation}
...
\begin{align}
\label{eq:SP}
-  \l \frac{dE}{dx}\r \sim \frac{n_\text{ion} Z^2 \alpha^2}{M v^2} \log {\l\frac{W_{\text{max}}}{W_{\text{min}}}\r}.
\end{align}
...
This yields a stopping power similar to that obtained with \eqref{eq:QED} up to $\OO(1)$ numerical factors that is roughly constant at high energies:
\begin{equation}
