...
Evidently, either a few $N \sim 1$ ultra-high energy particles or a $N \gg 1$ large number of lower energy particles can be released.
When applicable, we will distinguish the heating between these two scenarios. 
...
For a given species and interaction described by a stopping power $(dE/dx)$, we define a length scale $\lambda$ which is the characteristic range over which that species would predominantly deposit its energy to either stellar constituents or to additional secondaries. 
...
Finally, we discuss particle interactions in the WD due to the strong force. 
...
For our purposes, we will simply take the elastic cross section for nucleons and pions to be $\sigma_\text{el} \approx \text{b}$ when $E \lesssim 10 ~\MeV$, and ignore the energy loss due to elastic scatters at higher energies where inelastic processes will dominate. 
...
Below $\sim \GeV$ the photonuclear cross-section shows strong resonant behavior while above $\sim \GeV$, $\sigma_{\gamma A}$ is more or less constant in energy. 
...
As a result, we estimate the electronuclear stopping power to be of order $\sim E \alpha/l_{\gamma A}$.
