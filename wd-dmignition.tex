...
\paragraph{DM-SM Inelastic Scattering.}
...
t_\text{sg} &\sim 
  ...
  \l \frac{\sigma_{\chi A}}{10^{35} ~\cm^2} \r^{-1}.
\end{align}
However, capture progresses to a collapse only if $t_1 + t_2 < \tau_\x{WD}$, which requires
\begin{equation}
\label{eq:settle}
...
To illustrate the stringent nature of the collapse constraint, we will assume such a fiducial stable radius.  
