...
Properly, this DM velocity is described by a (boosted) Maxwell distribution peaked at the galactic virial velocity $v_\text{halo}$. 
...
For $m_\text{ion} \ll m_\chi$, the typical momentum transfer in an elastic scatter is $q \sim m_\text{ion} v_\text{esc} \sim 200 ~\MeV$. 
This corresponds to an energy transfer $q^2/m_\text{ion} \sim 10 ~\MeV$. 
...
Of course, the capture of DM in gravitating bodies is very well-studied \cite{Press:1985ug, Gould:1987ir}. 
However, the rate becomes slightly modified when the DM requires multiple scatters to lose the necessary energy for capture.
Parametrically the capture rate is of the form:
\begin{equation}
\Gamma_\text{cap} \sim \Gamma_\text{trans} \cdot \text{min}\left \{1, \bar{N}_\text{scat} \text{min}\{B^2,1\}\right\}, ~~~~ B^2 \equiv \frac{m_\text{ion} v_\text{esc}^2}{m_\chi v_\text{halo}^2}. 
\end{equation}
See \cite{Bramante:2017xlb} for a more detailed numerical calculation of this rate.  
Since the momentum transfer $q \sim 200 ~\text{MeV}$ is of order the inverse nuclear size, it is reasonable to expect the DM coherently scatters off all nucleons in a nucleus. 
...
For a DM velocity $v_\text{esc}$, we find that $F^2(q) \sim 0.1$. 
...
\end{equation}
Note that any DM candidate whose scattering cross section satisfies the direct detection constraint is necessarily capturing fewer than $1 \%$ of those DM which transit the star.
In this regime, the capture rate scales as $\Gamma_\text{cap} \propto \sigma_{\chi A}/m_\chi^2$. 
...
We now turn towards the rate of DM-DM collisions during the various stages of the above evolution. 
...
\end{equation}
If a collision does not occur during a time $t_2$, then a constraint can be derived from DM-DM collisions at the thermal radius. 
...
In fact, such a process yields novel bounds that also extend to lower DM masses, and will be discussed in detail in forthcoming work \cite{us}. 
