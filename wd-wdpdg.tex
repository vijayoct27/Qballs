...
Note that only those scatters which slow the incident particle are allowed by Pauli-blocking, so energy is always transfered from the incident to the Fermi electrons. 

As the electron speeds are much faster than the incident, a WD electron with momentum $\vec{q}$ will scatter to leading order with only a change in direction, $|\delta \vec{q}| \sim q$.
We again take the incident momentum $p \gtrsim p_F$, which is valid for all incident particles we consider. This results in an energy transfer $\omega$,
\begin{align}
\label{eq:NonRelEnergyTransfer}
  \omega = \left|\frac{p^2}{2 m} -
    \frac{\left(\vec{p} - \delta \vec{q}\right)^2}{2 m}\right|
    \sim \frac{q p}{2m}
\end{align}
For non-relativistic particles this is less than Fermi energy, so Pauli-blocking will be important. 
...
At leading order the electron is not aware of the small ion velocity, so scattering occurs with the recoilless, relativistic cross-sections-section 
\begin{align}
  \frac{d \sigma}{d \omega} = \frac{2 \pi Z^2 \alpha^2}{m} \frac{1}{\omega^2}
\end{align}
where $m$ is the incident mass. 
This yields a differential scattering rate,
\begin{align}
  \frac{d \Gamma}{d\omega} \sim n_e \frac{Z^2 \alpha^2}{m} \frac{1}{\omega}
\end{align}
and a stopping power
\begin{align}
  \frac{dE}{dx} =  \frac{1}{v} \int_0^E \; d\omega \; 
  n_\x{eff} \frac{d \sigma}{d \omega} \omega
  \sim n_e Z^2 \alpha^2 v
\end{align}
where $v = (E/m)^{1/2}$ is the incident particle velocity. 
