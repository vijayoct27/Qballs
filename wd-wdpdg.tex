...
Coulomb collisions with carbon ions provide the dominant mechanism by which electrons with energy $1~\MeV \lesssim E \lesssim 10~\MeV$ thermalize ions.
...
$\omega_\xmax$ is the maximum allowed energy transfer. 
...
We can estimate this by treating each ion in the medium as an oscillator with frequency $\Omega_p$ (i.e., an Einstein solid). 
The has two major effects: incident particles must transfer energy in multiples of $\Omega_p$ and the cross-section to transfer momentum $q$ is supressed by one factor of $q^2 / 2 m_\ion\Omega_p = \omega_\x{free}/\Omega_p$ for each oscilator quanta excited by the scatter.
$\omega_\x{free}$ is the energy transfer that would accompany an elastic scatter with momentum transfer $q$. 
The resulting stopping power is unchanged from the result~\eqref{eq:StoppingPowerOffIons} that ignored phonon excitations, as the increased energy transfer compenstates for the supressed cross-section:
\begin{align}
...
% NOTE: Do we want to comment here on other stopping scenairos? 
