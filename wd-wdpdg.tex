...
\begin{align}
  \frac{dE}{d x} &= \int_{0}^{\omega_\kin} d \omega \, n_\ion 
  \frac{d \sigma}{d \omega} \omega \nonumber\\
  \label{eq:StoppingPowerOffIons}
   &\approx \frac{2 \pi\, n_\ion Z^2 \alpha^2 }{m_\ion\beta^2} 
   \log\left( \frac{\omega_\kin}{\omega_\xmin} \right)
\end{align}
where the second line is valid if $\omega_\kin \gg \omega_\xmin$.


where $\omega_\kin$ is the maximum energy transfer is set by kinematics, and given by 
...
where $\omega_\xmin = \lambda_\TF^{-2} / 2 m_\ion$ is the energy transfer below which scatters are screened.


To understand the stopping power for Coulomb collisions with ions, let us first compute the cross section for incident energies $E \ll m_\ion$.
...
Furthermore, the energy transferred to the ion is given by $\omega = q^2 / 2m_\ion$.
Changing variables, we find
\begin{align}
  \label{eq:BornCrossSection}
\frac{d \sigma}{d \omega} = \frac{m_\ion}{2 \pi \beta^2} \left|\int d^3r \, e^{i\textbf{q}\cdot \textbf{r}}V(\textbf{r})\right|^2
\end{align}
...
\label{eq:CoulombOffIonsCrossSection}
\frac{d \sigma}{d \omega}
 % = \frac{m_\ion}{2 \pi \beta^2} \frac{Z^2 \alpha^2}{(q^2 + \lambda_\TF^{-2})^2}
  = \frac{2 \pi Z^2 \alpha^2}{m_\ion\beta^2} \frac{1}{(\omega + \omega_\xmin)^2}
\end{align}
where $\omega_\xmin = \lambda_\TF^{-2} / 2 m_\ion$.
So we see scatters that transfer momentum less than $ \sim \lambda_\TF^{-1}$ are screened.

Integrating this to obtain the stopping power, we find
\begin{align}
\frac{dE}{d x} &= \int_{0}^{\omega_\kin} d \omega \, n_\ion \frac{d \sigma}{d \omega} \omega \nonumber\\
\label{eq:StoppingPowerOffIons}
 &\approx \frac{2 \pi\, n_\ion Z^2 \alpha^2 }{m_\ion\beta^2} \log\left( \frac{\omega_\kin}{\omega_\xmin} \right)
\end{align}
...
If $\omega_F < \omega_\kin$, then the above integral should be taken with an upper limit of $\omega_F$ instead of $\omega_\kin$.

For higher energies $E \gtrsim m_\ion$ where recoil is important, the cross section is more complicated (e.g., see \cite{Schwartz}), but the overall stopping power is well approximated by extrapolating \eqref{eq:StoppingPowerOffIons} to higher energies.

One should also be careful about energy transfers smaller than the plasma frequency $\Omega_p \sim 1-10~\keV$, for which phonon excitations may be important.
A light incident particle with momentum $p$ and energy $E\ll m_\ion$ would transfer momentum $q\lesssim p$ and energy $\omega_\x{free} = q^2/2 m_\ion$ to a free ion.
We therefore expect phonon effects to be important when $p^2/2 m_\ion \lesssim \Omega_p$, and indeed one can check that the stopping power \eqref{eq:StoppingPowerOffIons} becomes dominated by energy transfers $\lesssim \Omega_p$ for this range of momenta.
...
The result is the same cross section \eqref{eq:CoulombOffIonsCrossSection} with an additional factor $q^2 / 2 m_\ion\Omega_p = \omega_\x{free}/\Omega_p$.
...
\begin{align}
n_\ion\cdot d\sigma\cdot\omega_\x{free} \to n_\ion \cdot d\sigma\,\frac{\omega_\x{free}}{\Omega_p}\cdot \Omega_p
\end{align}
...
% Scatters are rarer at incident momenta $p^2/2 m_\ion \lesssim \Omega_p$ where phonons matter, but each scatter transfers more energy so that the stopping power is equivalent.
