...
For incoming DM with asymptotic velocity $\approx v_\text{halo}$ (the peak of the Maxwell distribution), multiple scatters are required if
\begin{equation}
...
Ultimately we will be interested in the capture of DM leads to self-gravitational collapse, for which $m_\chi$ is sufficiently large that $B \lesssim 1$.
...
This occurs after a time
\begin{equation}
t_2 \sim \l \frac{m_\chi}{m_\text{ion}} \r \frac{1}{n_\text{ion} \sigma_{\chi A}} \frac{1}{v_\text{ion}} \left \{ 1+\log \l \frac{m_\chi}{m_\text{ion}}\r \right \} \approx 10^2 ~\text{yr} \l \frac{m_\chi}{10^{10} ~\GeV} \r \l \frac{\sigma_{\chi A}}{10^{-38} ~\cm^2} \r^{-1}. 
\end{equation}
...
\end{equation}
Naively, we expect that the rate of annihilations increases as the radius decreases.
This is true until the DM sphere collapses to $R_{\chi \chi}$ and has effectively depleted away, e.g. the timescale for annihilations matches the cooling timescale. 
Of course, the process of cooling via elastic nuclear scatters becomes slightly complicated if the DM sphere ever collapses to within a radius $n_\text{ion}^{-1/3} \approx 10^{-10} ~\text{cm}$. 
\textcolor{blue}{elaborate.}
...
\label{eq:decayDMcap}
\end{equation}
Since, \eqref{eq:decayDMcap} is independent of $\sigma_{\chi A}$ and scales inversely with $m_\chi$, the rate of captured DM decays in the WD is simply enhanced by a factor $\sim \l\frac{v_\text{esc}^3}{v_\text{halo}^2 v_\text{ion}} \r \approx 10^5$ compared to the rate of transiting ``wind" decays \eqref{eq:decayDM}.  
...
These are obviously more stringent than that of the wind scenario---for decays, it is simply enhanced by a numerical factor $\approx 10^{5}$. 
