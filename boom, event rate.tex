...
Since $v \ll v_\text{esc}$, we demand that the DM lose a fraction $(v/v_\text{esc})^2$ of its energy to eventually become stopped in the star. 
...
%\begin{equation}
%\frac{dn_\chi}{dv} \sim \frac{\rho_\chi}{m_\chi} \frac{v}{v_\text{halo}^2},
%\end{equation}
%appropriately normalized. 
If every DM that passes through the WD is captured, then the capture rate is simply \eqref{eq:TransitFluxCondition}. 
...
For $m_\chi \gg m_\text{ion}$, the typical momentum transfer in an elastic scatter is $q \sim \mu_{A} v_\text{esc} \approx 200 ~\MeV$, where $\mu_{A}$ is the reduced mass of the DM-nuclei system. 
...
\end{equation}
In a WD, the typical number of scatters expected is of order
\begin{equation}
N_\text{scatters} \sim n_\text{ion} \sigma_{\chi A} R_\text{WD}. 
\end{equation}
In the case of multiple scatters $N_\text{scatters} \gg 1$, the energy loss per distance travelled at velocity $\sim v_\text{esc}$ is
\begin{equation}
\left( \frac{d E}{d x} \right)_\text{SP} \sim n_\text{ion} \sigma_{\chi A} m_\text{ion} v_\text{esc}^2.
\end{equation}
...
In this case, one can show \cite{Gould} that the capture rate is modified to
\begin{equation}
\Gamma_\text{cap} \sim \Gamma_\text{transit} \cdot \text{min} \left \{ \l \frac{v_\text{cap}}{v_\text{halo}} \r^2, 1\right \}. 
\end{equation}

Evidently, the assumption that the scatters responsible for slowing the DM are not sufficient to blow up the WD \eqref{eq:transitexplosion} is a valid for cross sections
\begin{equation}
...
For our purposes, we simply demand that the settle time is shorter than the age of the WD:
\begin{equation}
...
Also, DM masses greater than $\sim 10^{21} ~\GeV$ will collapse before accumulating more than a single particle $N_\text{crit} < 1$, which is ultimately a limiting case of the boom condition \eqref{eq:multcolboom} that requires at least one annihilation during the collapse. 
...
\end{equation}
As an example, consider the case of efficient capture $\Gamma_\text{cap} \sim \Gamma_\text{transit}$ and a heating length within the trigger size $L_0 \sim \lambda_T$. 
...
Of course, the number of DM-DM collisions $N_\text{multi}$ that can be counted as a single heating event depends on where we examine the collapse. 
...
As an example, suppose $\sigma_{\chi \chi}$ is independent of position or velocity. 
...
%\end{equation}
Note that if the DM number is substantially depleting below $R_{\chi \chi}$ according to \eqref{eq:DMcollapsedeplete}, then one can show that a BH never forms. 
...
If either $\sigma_{\chi \chi} \propto 1/v$ or $\sigma_{\chi \chi} \propto 1/v^2$, then the optimum radius is instead given by 
\begin{equation}
R_* = \text{max}\{R_{\chi \chi}, R_\text{BH}\},
\end{equation}
taking the collapse to be stabilized at a black hole. 
For simplicity, we employ this optimum radius in the expression \eqref{eq:nmulti} for $N_\text{multi}$. 
It is straightforward to check that $N_\text{multi}$ is at most $N_\text{crit}$, the total number of DM particles collapsing, and saturates this value for $R_* \sim R_{\chi \chi}$, with $ R_{\chi \chi} < L_\text{heat}$ and $R_{\chi \chi}/v_\text{ff} < \tau_\text{diff}$. 
