...
This annihilation rate is ultimately dominated by the DM density inside the star with orbits near the thermal radius, and the depletion of in-falling DM can be ignored as long as:
...
Soon after, further collapse will proceed as a result of the DM dissipating its energy through elastic scatters and reducing its typical orbital radius.
...
\end{align}
This condition is ultimately be satisfied for any DM mass and annihilation cross section which release enough energy to ignite the WD during a collapse. 
%The enclosed WD mass is also dropping by $M_\text{WD}(r) \propto r^3$ during the collapse, so if the DM cloud depletes as a stronger function of radius then the collapse will halt below $R_{\chi \chi}$. 
...
Of course this depends on the radius of the collapsing DM sphere, but there necessarily exists an optimum radius $R_*$ at which the annihilation rate is maximized.
...
\begin{equation}
\frac{N_\text{crit}}{R_\text{BH}^3} \sigma_{\chi \chi} \frac{m_\chi}{\rho_\text{WD} \sigma_{\chi A}} < 1. 
\end{equation}
