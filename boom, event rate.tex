...
For simplicity we employ this optimum radius in the expression \eqref{eq:nmulti} for $N_\text{multi}$ from now on.  
...
The region in Figure \ref{fig:multicapture} are the set of all paramters which satisfy \eqref{eq:collapsecondition} and \eqref{eq:multcolboom}----with $N_\text{multi}$ given by \eqref{eq:nmulti}---assuming that the DM collapse is stabilized by formation of a BH. 
Additionally, one can check that the conditions \eqref{eq:steadycollect} and \eqref{eq:xicondition} are also satisfied for such parameters. 
...
A naive estimate for the annihilation cross section of such type of DM is of order
\begin{equation}
