...
The DM eventually thermalizes to an average velocity
\begin{equation}
...
%\end{align}
For our purposes, we simply require that the time for DM to thermalize is shorter than the age of the WD
\begin{equation}
t_1 + t_2 < \tau_\text{WD}.
\end{equation}
Note that the settling DM constitutes a number density of DM throughout the WD volume as well as outside the star. 
%\begin{equation}
...
This annihilation rate is dominated by the DM density inside the star with orbits near the thermal radius
\begin{equation}
\label{eq:infallrate}
...
After a settling time has passed, DM will begin steadily accumulating at the thermal radius $R_\text{th}$.
...
\end{equation}
If $\Gamma_\text{cap} \sim \Gamma_\text{trans}$ in a local DM density $\rho_\chi \sim 0.4 ~\GeV/\cm^3$, this occurs for DM masses above $\sim 10^{17} ~\GeV$. 
In this scenario, the DM cloud at $R_\text{th}$ will continue to collect more DM until a saturation number $N_\text{crit}^{1/3} (\Gamma_\text{cap} t_\text{col})^{2/3}$ greater than $N_\text{crit}$, at which point the timescale for free-fall matches the timescale for collection.
...
\begin{equation}
\label{eq:nocollapse}
N_\text{mult} \sim \l \frac{\text{min}\{N_\text{eq}, N_\text{life}\}}{R_\text{th}^3} \r^2 \sigma_{\chi \chi} v_\text{th} \text{max}\{\lambda_T,L_0\}^3 \tau_\text{diff}. 
\end{equation}
...
N_\text{mult} \sim \int \l \frac{N}{r^3}\r^2 \sigma_{\chi \chi} \text{min}\{L_\text{heat}, r\}^3 dr, ~~~~ L_\text{heat} \equiv \text{max}\{\lambda_T, L_0\}
\end{equation}
...
We turn towards constraints on DM interactions in the capture scenario. 
In Figures \ref{fig:singlecapture} and \ref{fig:multicapture}, we show the constraints on $\sigma_{\chi \chi}$ assuming that the DM is efficiently captured, i.e. $\Gamma_\text{cap} \sim \Gamma_\text{trans}$, and the settle time is less than the WD lifetime. 
The results are valid for any SM annihilation products which deposit their energy compactly upon release within a trigger size $\lambda_T$.
We take care to distinguish between whether a single collision can blow up the star or multiple collisions are required, although both bounds are derived using the gravitational core collapse of DM in a WD. 
%The plotted region is the set of all parameters which satisfy \eqref{eq:collapsecondition} and \eqref{eq:multcolboom}----with $N_\text{mult}$ given by \eqref{eq:nmulti}---assuming that the DM collapse is stabilized by formation of a BH. 
%One can qualitatively understand the range of excluded cross sections as follows:
...
\caption{Constraints on DM-DM annihilation cross-section into SM particles which deposit their energy compactly within a trigger size $\lambda_T$ during self-gravitational collapse in a WD. Bounds come from observation of a single $1.25~M_{\astrosun}$ WD assuming efficient capture of the DM and settling time (see text). We also take the DM collapse is stabilized by formation of a BH.}
\label{fig:multicapture}
\end{figure}
As in Section \ref{sec:previous}, it is straightforward to specify to the case of DM capture via elastic scatters. 
We consider a generic class of heavy WIMP (``WIMPzilla") DM models. 
...
In order to not be ruled out by XENON, such a DM must be heavier than $m_\chi \gtrsim 10^{10} ~\GeV$. 
...
\begin{equation}
\sigma_{\chi \chi} v \sim \frac{1}{8\pi} \frac{g_W^4}{m_\chi^2} ~~ (\text{naive}).
\end{equation}
...
\end{equation}
Thus, DM masses $m_\chi > 10^{11}$ will have a heating length larger than the trigger size of a $1.25~M_{\astrosun}$ WD.
However, we expect that at such high energies there should be a considerable branching fraction for direct production of quarks from DM-DM annihilations via the collinear singularity. 
Since hadrons stop efficiently in the WD medium \textcolor{blue}{elaborate}, the heating length for these processes is simply $\lambda_T$. 
...
Such bounds are complementary to that obtained in the wind scenario.  
\begin{figure}
\includegraphics[scale=.35]{GZcapture.pdf}
\caption{Constraints on `WIMPzilla DM models which elastically scatter off nuclei through Z boson-exchange and annihilate into electroweak gauge bosons. Bounds come from observation of a single $1.25~M_{\astrosun}$ WD.
