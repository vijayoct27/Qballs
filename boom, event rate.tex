...
As an example, first consider the case where $\sigma_{\chi \chi}$ is independent of position or velocity. 
...
If $\sigma_{\chi \chi} \propto 1/v$, then a similar argument results in an optimum radius:
\begin{equation}
R_* = \text{max}\{R_{\chi \chi}, R_\text{sta}\}.
\end{equation}
The same holds true for cross sections $\sigma_{\chi \chi} \propto 1/v^2$, in which case one can show that there is even no collapse to radii below $R_{\chi \chi}$ due to excessive depletion.  
...
The region in Figure \ref{fig:multicapture} are the set of all points which satisfy \eqref{eq:collapsecondition} and \eqref{eq:multcolboom}----with $N_\text{multi}$ given by \eqref{eq:nmulti}---assuming that the DM collapse is stabilized by formation of a BH. 
Additionally, one can check that the conditions \eqref{eq:steadycollect} and \eqref{eq:xicondition} hold if \eqref{eq:collapsecondition} and \eqref{eq:multcolboom} are satisfied.
...
\caption{Constraints on a generic class of DM models which elastically scatter off nuclei through Z boson-exchange and annihilate into W bosons. Bounds come from observation of a single $1.25~M_{\astrosun}$ WD.
