...
\end{equation}
If $N_\text{scat} < 1$, then $N_\text{scatters}$ is the probability for a \emph{single} scatter to occur during the transit. 
...
This annihilation rate is dominated by the DM density inside the star and is of order
\begin{equation}
\label{eq:infallrate}
...
For efficient capture $\Gamma_\text{cap} \sim \Gamma_\text{trans}$ and a local DM density $\rho_\chi \sim 0.4 ~\GeV/\cm^3$, this takes place if the DM is sufficiently heavy $m_\chi \gtrsim 10^{17} ~\GeV$. 
...
\end{equation}
Thus the initial number of collapsing DM particles simply $\sim \text{max}\{N_\text{crit}, N_\text{sat}\}$. 
