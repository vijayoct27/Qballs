...
\end{align}
This can only be achieved if the time to collect a critical mass of DM is shorter than the time for annihilations to deplete this mass sufficiently \emph{and} shorter than WD lifetime. 
In other words:
\begin{equation}
...
\end{align}
Demanding that the free-fall time is less than the annihilation time of a single DM particle results in an additional condition:
\begin{align}
...
For composite DM, it is reasonable to suspect that the collapse stabilizes into a core of radius $R_\text{sta}$ larger than the Schwarzschild radius $R_\text{sch} \sim G m_\chi N_\text{crit}$ due to some unknown physics. 
...
If the conditions for collapse are met, the number of DM particles in the collapsing core can be written as a function of collapse radius:
\begin{equation}
N(r) \sim \frac{N_\text{crit}}{1 + \xi \l \frac{R_\text{th}}{r}\r^2}. 
\end{equation}
Thus, the effect of annihilations on the collapsing DM mass becomes important at a radius,
\begin{equation}
R_{\chi \chi} \sim R_\text{th} \sqrt{\xi},
\end{equation}
below which the mass of collapsing DM depletes as $N \propto r^2$. 

We now turn towards the rate of explosive collisions and decays for captured DM in the case that a single collision or decay is sufficient to ignite the star. 
