...
However, there is an interesting alternative if the DM is captured that allows DM masses below $10^{16} ~\GeV$ to be constrained. 
Multiple collisions in a sufficiently small region can occur rapidly enough to be counted as a single heating event.
This is similar in nature to a transit heating event where individual scatters do not necessarily deposit $\Eboom$ but the sum total of multiple scatters across a transit length $\lambda_T$ can release this amount and satisfy \eqref{eq:transitexplosion}. 
Thus, even if a single DM-DM collision is unable to ignite the star, the sum total of many collisions can still result in a SN if the following boom condition is satisfied:
\begin{equation}
...
\end{equation}
$N_\text{mult}$ is the number of collisions happening within a region of size $\lambda_T^3$ or smaller during a diffusion time $\tau_\text{diff}$.
The precise expression for $N_\text{mult}$ will depend on the evolution of the DM in the star at the desired instance.
Unlike the condition for a single collision \eqref{eq:coldecay}, the condition for multiple collisions to be explosive \eqref{eq:multcolboom} necessarily depend on the collision cross section and requires a number of assumptions on the DM interactions. 
...
The physics of DM capture can be made more precise for a specific interaction: the simplest scenario is a spin-independent, elastic scatter off ions characterized by cross section $\sigma_{\chi A}$. 
...
If the DM has an initial relative velocity with the WD $v \approx v_\text{halo}$, then DM masses $m_\chi \gtrsim 10^3 ~\GeV$ will require multiple scatters to lose the necessary fraction of its energy. 
...
Initially there is a timescale for DM to slow down to the thermal velocity $v_\text{th}$ and settle to the thermal radius.
If the DM thermalizes rapidly in the star e.g. via inelastic scatters, this time is at most
\begin{align}
\label{eq:tdrift}
  t_\text{drift} \lesssim \frac{R_\text{WD}}{v_\text{th}}
  \approx 50 ~\text{yr} ~ \l \frac{m_\chi}{10^{16} ~\GeV} \r^{1/2}. 
\end{align}
However, the settle time can be explicitly calculated for a given interaction.
Consider the case of DM-nuclear elastic scatters. 


Once formed, the DM cloud at $R_\text{th}$ will continue to collect DM until it reaches a critical core mass \eqref{eq:mcore} in a time
\begin{align}
\label{eq:tcol}
