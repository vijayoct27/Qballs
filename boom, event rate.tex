...
\end{equation}
In fact, this condition is independent of $m_\chi$ and translates to an upper bound on the cross section $\sigma_{\chi \chi} < 10^{-13} ~\text{cm}^2$. 
...
\caption{Constraints on DM-DM annihilation cross-section into SM particles which deposit their energy compactly within a trigger size $\lambda_T$ during self-gravitational collapse in a WD. Bounds come from observation of a single $1.25~M_{\astrosun}$ WD assuming efficient capture of the DM ($\Gamma_\text{transit} \sim \Gamma_\text{capture}$). We also assume the DM collapse is stabilized by formation of a BH.}
...
Of course, the constraints on multiple DM collisions in a collapse can be made more concrete for a specific model of the DM.
...
A concrete example considered by \textcolor{blue}{keisuke and friends} is a type of heavy WIMPzilla DM they call ``GUTzilla".
This DM model is a hypercharged electroweak doublet and thus scatters with a cross section \eqref{eq:Zscattering}. 
Gutzilla is restricted to the mass range $m_\chi \sim 10^{9} - 10^{12} ~\GeV$---the lower end of this range is set by evading direct detection constraints, while the upper end is more model-specific.
Of course, GUTzilla has an annihilation cross section into W bosons.
The naive estimate for this cross section would be of order
\begin{equation}
\sigma_{\chi \chi} v \sim \frac{\alpha_W^2}{m_\chi^2}.
\end{equation}
In fact GUTzilla is more complicated, although it is expected that there is still an upper limit (the so-called unitarity limit) on the annihilation cross section due to its point-like nature:
\begin{equation}
\sigma_{\chi \chi} v \lesssim \frac{4 \pi}{m_\chi^2} \frac{1}{v}.
\end{equation}
The W boson annihilation products decay predominantly to quarks with a decay length
\begin{equation}
