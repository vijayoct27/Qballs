...
\label{eq:Ncore}
    ...
    &\approx 10^{12} ~\GeV \l \frac{10^{16} ~\GeV}{m_\chi} \r^{5/2}. \nonumber
\end{align}
...
For composite DM, it is reasonable to suspect that the collapse stabilizes into a core larger than the Schwarzschild radius, although this depends on unknown physics. 
...
The expectation is that there exists an optimal value of the lower radius $R_*$ for which $N_\text{multi}$ is maximized. 
However, even without knowing the details of this optimum choice, we can evaluate the integral \eqref{eq:capintegral}. 
...
\end{equation}
The questions is: what is the radius $R_*$ at which the explosiveness of multiple collisions is optimal?
Ultimately, this depend on the DM mass $m_\chi$ and DM-DM collision cross section $\sigma_{\chi \chi}$. 
If $R_{\chi \chi} > \lambda_T$, then $N_\text{multi}$ is maximized at $R_* = \lambda_T$.
On the other hand, if $R_{\chi \chi} > \lambda_T$ then $N_\text{multi}$ is maximized at $R_* = R_{\chi \chi}$.
However, assuming no other stabilizing force, the DM cannot collapse below the Schwarzschild radius of the collapsing mass. 
This is at most
\begin{equation}
R_\text{sch} \sim G m_\chi N_\text{crit} \approx 5 \times 10^{-24} ~\cm \l \frac{10^{16} ~\GeV}{m_\chi} \r^{3/2},
\end{equation}
but will be parametrically smaller if the DM number is substantially depleting according to \eqref{eq:DMcollapsedeplete}. 
Thus, the optimum radius is determined as follows:
\begin{equation}
R_* = \text{max}\{\text{min}\{\lambda_T, R_{\chi \chi}\}, R_\text{sch}\}.
\end{equation}
