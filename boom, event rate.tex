...
\begin{equation}
t_\text{cool} \sim \frac{m_\chi}{\rho_\text{WD} \sigma_{\chi A}} \frac{1}{\text{max}\{{v_\text{ion},v\}}}, ~~~~ v \sim \sqrt{\frac{G N_\text{sg} m_\chi}{r}}.
\end{equation}
...
Once the accumulated DM reaches $N_\text{sg}$, evolution of the DM profile can either be collapse or further collection.
The later scenario occurs if the collapse time is greater than the time to collect a critical number of DM particles:
\begin{equation}
N_\text{sg} < \Gamma_\text{cap} t_\text{col}.
\end{equation}
This is true for DM masses above $\sim 10^{14} ~\GeV$, independent of scattering cross section $\sigma_{\chi A}$. 
%Here the DM sphere at $R_\text{th}$ will continue to collect more DM until a saturation number $\sim N_\text{sg}^{1/3} (\Gamma_\text{cap} t_\text{col})^{2/3}$ greater than $N_\text{sg}$, at which point the timescale for free-fall matches the timescale for collection.
...
\begin{equation}
\label{eq:nmulti}
N_\text{mult} \sim \l \frac{N_\text{sg}}{R_*^3}\r^2  \sigma_{\chi \chi} v_\text{ff}(R_*) ~\text{min}\{L_\text{heat}, R_*\}^3 \tau_\text{diff},
\end{equation}
